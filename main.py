...
# =========================================================
# MANTENIMIENTO SEGURO: reset de selectores privados
# Borra SOLO los vínculos privados HOT/DJ para reparar menús privados.
# No toca /data/state.json, recurrentes, validación, listas, biblioteca ni configuración del grupo.
# =========================================================
async def resetselectores_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    user_id = int(update.effective_user.id)
    if ADMIN_IDS and user_id not in ADMIN_IDS:
        await update.message.reply_text("⛔ Solo ADMIN_IDS puede ejecutar /resetselectores.")
        return

    deleted = []
    errors = []

    # Limpiar memoria y archivos de vínculos privados DJ/HOT.
    try:
        global DJ_PRIVATE_GROUPS
        DJ_PRIVATE_GROUPS = {}
        if DJ_PRIVATE_GROUPS_PATH.exists():
            DJ_PRIVATE_GROUPS_PATH.unlink()
            deleted.append(str(DJ_PRIVATE_GROUPS_PATH))
        else:
            deleted.append(str(DJ_PRIVATE_GROUPS_PATH) + " (no existía)")
    except Exception as e:
        logger.exception("No se pudo limpiar DJ_PRIVATE_GROUPS")
        errors.append(f"DJ: {e}")

    try:
        global HOT_PRIVATE_GROUPS
        HOT_PRIVATE_GROUPS = {}
        if HOT_PRIVATE_GROUPS_PATH.exists():
            HOT_PRIVATE_GROUPS_PATH.unlink()
            deleted.append(str(HOT_PRIVATE_GROUPS_PATH))
        else:
            deleted.append(str(HOT_PRIVATE_GROUPS_PATH) + " (no existía)")
    except Exception as e:
        logger.exception("No se pudo limpiar HOT_PRIVATE_GROUPS")
        errors.append(f"HOT: {e}")

    # Limpieza suave de acciones pendientes privadas que pueden quedarse enganchadas.
    try:
        for key in list(PENDING_ACTIONS.keys()):
            if str(key).startswith(("cfg:", "hot:", "dj:")):
                PENDING_ACTIONS.pop(key, None)
    except Exception:
        pass

    text = (
        "✅ <b>Selectores privados reiniciados</b>\n\n"
        "He borrado solo los vínculos privados de configuración HOT/DJ.\n"
        "No he tocado <code>/data/state.json</code>, así que se conservan recurrentes, validación, listas, biblioteca y configuraciones.\n\n"
        "<b>Archivos tratados:</b>\n" + "\n".join(f"• <code>{h(x)}</code>" for x in deleted)
    )
    if errors:
        text += "\n\n<b>Errores:</b>\n" + "\n".join(f"• {h(e)}" for e in errors)
    text += (
        "\n\n<b>Ahora haz esto:</b>\n"
        "1) En cada grupo ejecuta <code>/hotgrupo</code> y <code>/djgrupo</code>.\n"
        "2) Vuelve al privado del bot.\n"
        "3) Ejecuta <code>/ajustes</code>, <code>/hotconfig</code> o <code>/djconfig</code>."
    )
    await update.message.reply_html(text)

async def debugselectores_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    user_id = int(update.effective_user.id)
    if ADMIN_IDS and user_id not in ADMIN_IDS:
        await update.message.reply_text("⛔ Solo ADMIN_IDS puede ejecutar /debugselectores.")
        return
    dj = DJ_PRIVATE_GROUPS.get(str(user_id), {}) if isinstance(DJ_PRIVATE_GROUPS, dict) else {}
    hot = HOT_PRIVATE_GROUPS.get(str(user_id), {}) if isinstance(HOT_PRIVATE_GROUPS, dict) else {}
    state_chats = []
    for cid in sorted(STATE_CACHE.keys()):
        try:
            if int(cid) < 0:
                state_chats.append(f"{cid}: {cfg_value(int(cid), 'chat_title', '') or cid}")
        except Exception:
            pass
    text = (
        "🧪 <b>Debug selectores privados</b>\n\n"
        f"<b>Usuario:</b> <code>{user_id}</code>\n"
        f"<b>DJ_PRIVATE_GROUPS_PATH:</b> <code>{h(DJ_PRIVATE_GROUPS_PATH)}</code> existe: <b>{DJ_PRIVATE_GROUPS_PATH.exists()}</b>\n"
        f"<b>HOT_PRIVATE_GROUPS_PATH:</b> <code>{h(HOT_PRIVATE_GROUPS_PATH)}</code> existe: <b>{HOT_PRIVATE_GROUPS_PATH.exists()}</b>\n\n"
        f"<b>DJ guardado para ti:</b> <code>{h(json.dumps(dj, ensure_ascii=False)[:1200])}</code>\n\n"
        f"<b>HOT guardado para ti:</b> <code>{h(json.dumps(hot, ensure_ascii=False)[:1200])}</code>\n\n"
        "<b>Grupos en state.json:</b>\n" + ("\n".join(f"• {h(x)}" for x in state_chats[:30]) or "No hay grupos detectados")
    )
    await update.message.reply_html(text)
...
def build_application() -> Application:
    ...
    application.add_handler(command_handler("resetselectores", resetselectores_command))
    application.add_handler(command_handler("debugselectores", debugselectores_command))
...
# =========================
# SELECTORES PRIVADOS SAFE PATCH
# Recupera menús privados aunque los vínculos HOT/DJ se hayan limpiado.
# Usa grupos existentes en /data/state.json y callbacks propios safeopen|.
# =========================

def _safe_state_groups_for_private() -> Dict[int, str]:
    try:
        load_all_states()
    except Exception:
        pass
    ...
    for chat_id, state in list(STATE_CACHE.items()):
        try:
            cid = int(chat_id)
            # Solo grupos/supergrupos; evita IDs positivos de usuarios privados.
            if cid >= 0:
                continue
            title = str(cfg_value(cid, "chat_title", "") or getattr(state, "assigned_dj_name", "") or cid)
...
def _safe_group_selector_markup() -> InlineKeyboardMarkup:
    rows: List[List[InlineKeyboardButton]] = []
    for cid, title in _safe_state_groups_for_private().items():
        rows.append([InlineKeyboardButton(f"⚙️ {title}", callback_data=f"safeopen|{cid}")])
    rows.append([InlineKeyboardButton("🔄 Actualizar lista", callback_data="safeopen|refresh")])
    return InlineKeyboardMarkup(rows)


# Sobrescribe el selector del menú /ajustes para no usar cfg|open en la lista inicial.
def admin_private_chat_list_markup() -> InlineKeyboardMarkup:  # type: ignore[no-redef]
    return _safe_group_selector_markup()


async def safe_ajustes_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo ADMIN_IDS puede abrir la configuración privada.")
    ...
    if update.effective_chat and update.effective_chat.type != "private":
        await update.message.reply_text("Escríbeme /ajustes por privado para configurar el grupo sin ensuciar el chat.")
        return
    groups = _safe_state_groups_for_private()
    if not groups:
        await update.message.reply_text("No veo grupos en /data/state.json. Ejecuta /plan una vez dentro del grupo y vuelve aquí.")
        return
    await update.message.reply_html("<b>Elige el grupo que quieres configurar:</b>", reply_markup=_safe_group_selector_markup())


async def safeopen_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    q = update.callback_query
    ...
    try:
        ...
        pass
    data = q.data or ""
    if data == "safeopen|refresh":
        try:
            await q.message.edit_text("<b>Elige el grupo que quieres configurar:</b>", parse_mode=ParseMode.HTML, reply_markup=_safe_group_selector_markup())
        except Exception:
            await context.bot.send_message(q.message.chat_id, "<b>Elige el grupo que quieres configurar:</b>", parse_mode=ParseMode.HTML, reply_markup=_safe_group_selector_markup())
        return
    try:
    ...
    try:
        if not await is_admin(context, chat_id, q.from_user.id):
            await q.answer("No constas como administrador de ese grupo.", show_alert=True)
            return
    except Exception:
        # Si falla la comprobación por permisos/Telegram, dejamos entrar a ADMIN_IDS global.
        if not is_global_admin_user(q.from_user.id):
            await q.answer("No puedo comprobar permisos en ese grupo.", show_alert=True)
            return
    try:
        # Guarda título si falta y vincula HOT/DJ para que hotconfig/djconfig también arranquen.
        title = _safe_state_groups_for_private().get(chat_id, str(chat_id))
        cfg_set(chat_id, "chat_title", title)
        try:
            hot_link_private_group(q.from_user.id, chat_id, title)
        except Exception:
            pass
        try:
            dj_link_private_group(q.from_user.id, chat_id, title)
        except Exception:
            pass
        await context.bot.send_message(
            q.message.chat_id,
        ...
        logger.exception("safeopen no pudo abrir panel privado")
        await context.bot.send_message(q.message.chat_id, f"❌ Error abriendo panel: {h(e)}", parse_mode=ParseMode.HTML)


async def autovincularselectores_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("⛔ Solo ADMIN_IDS puede ejecutar /autovincularselectores.")
        return
    user_id = int(update.effective_user.id)
    groups = _safe_state_groups_for_private()
    if not groups:
        await update.message.reply_text("No veo grupos en state.json. Ejecuta /plan en el grupo primero.")
        return
    linked = []
    for cid, title in groups.items():
        try:
            hot_link_private_group(user_id, cid, title)
        except Exception:
            logger.exception("No se pudo autovincular HOT %s", cid)
        try:
            dj_link_private_group(user_id, cid, title)
        except Exception:
            logger.exception("No se pudo autovincular DJ %s", cid)
        linked.append(f"• <code>{cid}</code>: {h(title)}")
    await update.message.reply_html(
        "✅ <b>Selectores autovinculados</b>\n\n"
        + "\n".join(linked[:30])
        + "\n\nAhora prueba <code>/ajustes</code>, <code>/hotconfig</code> y <code>/djconfig</code> por privado."
    )


# Refuerza /hotconfig: si no hay grupos HOT vinculados, usa los grupos de state.json.
_old_hotconfig_command_safe = hotconfig_command
async def hotconfig_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[no-redef]
    if update.effective_chat and update.effective_chat.type == "private" and update.effective_user:
        if not hot_private_groups_for(update.effective_user.id):
            for cid, title in _safe_state_groups_for_private().items():
                try:
                    hot_link_private_group(update.effective_user.id, cid, title)
                except Exception:
                    pass
    await _old_hotconfig_command_safe(update, context)


# Refuerza /djconfig: si no hay grupos DJ vinculados, usa state.json.
_old_djconfig_command_safe = djconfig_command
async def djconfig_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[no-redef]
    if update.effective_chat and update.effective_chat.type == "private" and update.effective_user:
        if not dj_private_groups_for(update.effective_user.id):
            for cid, title in _safe_state_groups_for_private().items():
                try:
                    dj_link_private_group(update.effective_user.id, cid, title)
                except Exception:
                    pass
    await _old_djconfig_command_safe(update, context)


_old_build_application_safe = build_application
def build_application() -> Application:  # type: ignore[no-redef]
    app = _old_build_application_safe()
    # Handlers prioritarios y callbacks propios, añadidos al final con grupos separados.
    app.add_handler(CommandHandler(["ajustes", "configuracion", "config", "admin"], safe_ajustes_command), group=-20)
    app.add_handler(CommandHandler("autovincularselectores", autovincularselectores_command), group=-20)
    app.add_handler(CallbackQueryHandler(safeopen_callback, pattern=r"^safeopen\|"), group=-20)
    return app
