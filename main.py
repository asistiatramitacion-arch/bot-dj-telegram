...
async def is_controller(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int) -> bool:
    ...
    return state.assigned_dj_id == user_id or await is_admin(context, chat_id, user_id)
...
async def controller_users_text(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> str:
    ...
    async def resolve_name(user_id: Optional[int], fallback: str = "") -> str:
        ...
        try:
            ...
            pass
        return fallback or f"ID {user_id}"

    if state.assigned_dj_id:
        dj_name = await resolve_name(state.assigned_dj_id, state.assigned_dj_name or f"ID {state.assigned_dj_id}")
        lines.append(f"🎧 DJ asignado: <b>{dj_name}</b> — <code>{state.assigned_dj_id}</code>")
    else:
        lines.append("🎧 DJ asignado: <b>Sin asignar</b>")

    if ADMIN_IDS:
        lines.append("")
        lines.append("<b>IDs registrados con control:</b>")
        for admin_id in sorted(ADMIN_IDS):
            admin_name = await resolve_name(admin_id, f"ID {admin_id}")
            lines.append(f"• {admin_name} — <code>{admin_id}</code>")
    else:
        lines.append("")
        lines.append("<i>No hay ADMIN_IDS configurados. Solo el DJ asignado y admins del chat pueden controlar.</i>")
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "menu_panel":
        if not await is_controller(context, chat_id, user_id):
            await query.answer("Solo el DJ asignado o el ID registrado puede abrir el panel.", show_alert=True)
            return
        state.dj_mode = True
    ...
    if data in ("panel_live_toggle", "panel_pause_resume"):
        ...
        try:
            ...
            if live_result and not state.queue and not state.now_playing:
                await query.answer("LIVE ON · Videochat iniciado", show_alert=False)
            else:
