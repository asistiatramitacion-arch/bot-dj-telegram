...
def parse_recurring_when(raw: str) -> Dict[str, Any]:
    ...
    raise ValueError("Formato de tiempo no válido. Usa: cada 60m, cada 1h, cada hora o diario 21:30")
            ...
            try:
                ...
                for row in recurring_list(chat_id):
                    if recurring_due(row, now):
                        ...
                        if row.get("schedule", {}).get("type") == "daily":
                            row["last_day"] = time.strftime("%Y-%m-%d", time.localtime(now))
                        save_all_states()
...
async def pregonero_auto_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if arg in ("off", "desactivar", "no"):
        ...
        return
    if not _re_v7.match(r"^[0-2]?\d:[0-5]\d$", arg):
        await update.message.reply_html("Uso:\n<code>/pregoneroauto 21:30</code>\n<code>/pregoneroauto off</code>")
        return
    cfg["pregonero_auto_enabled"] = True
    cfg["pregonero_auto_time"] = arg
    save_all_states()
    await update.message.reply_text(f"📣 Pregonero automático activado a las {arg}.")
        ...
        for chat_id in list(STATE_CACHE.keys()):
            try:
                ...
                if not bool(cfg.get("pregonero_auto_enabled", False)):
                    continue
                if str(cfg.get("pregonero_auto_last_day", "")) == day:
                    continue
                ...
                if (lt.tm_hour, lt.tm_min) >= (hh, mm):
                    ...
                    cfg["pregonero_auto_last_day"] = day
                    save_all_states()
            except asyncio.CancelledError:
                raise
            except Exception:
...
# =========================================================
# V75 - DJ Fácil / DJ-PRO con temporizadores y panel aislado
# =========================================================
...
def build_application() -> Application:  # type: ignore[override]
    ...
    logger.info('V75 DJ Fácil/DJ-PRO con cierre, siguiente y temporizadores cargado')
    return app
