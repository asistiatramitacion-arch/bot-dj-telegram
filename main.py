...
SEARCH_TRIGGER = os.getenv("SEARCH_TRIGGER", "@VoiceShazamBot ").strip()
...
CONTROL_REFRESH_TASKS: Dict[int, asyncio.Task] = {}
...
def remaining_seconds(state: ChatState) -> int:
    if not state.now_playing:
        return 0
    duration = int(state.now_playing.get("duration") or 0)
    started = int(state.playback_started_at or 0)
    if duration <= 0:
        return 0
    if state.paused or started <= 0:
        return duration
...
def control_header(state: ChatState) -> str:
    ...
    remaining = fmt_duration(remaining_seconds(state))
    return (
        ...
        f"⏱️ Restante: <b>{remaining}</b>\n"
        f"⏭️ Auto-sig: <b>{state.autoplay_offset}</b>\n"
...
def queue_text(state: ChatState, page: int = 0) -> str:
    ...
    if not state.queue:
        lines.append("\nLa cola está vacía.")
        return "\n".join(lines)
    page_items, page, total_pages = paginate_items(state.queue, page)
    lines.append("")
    ...
    for offset, item in enumerate(page_items, start=1):
        ...
        lines.append(f"{start_idx + offset}. {track.title}{suffix}")
    lines.append(f"\nPágina <b>{page + 1}/{total_pages}</b>")
    return "\n".join(lines)
...
def queue_markup(state: ChatState, page: int = 0) -> InlineKeyboardMarkup:
    ...
    for offset, _ in enumerate(page_items):
        ...
        row: List[InlineKeyboardButton] = [InlineKeyboardButton("▶️", callback_data=f"q|p|{idx}|{page}")]
        if idx > 0:
...
def library_text(state: ChatState, page: int = 0) -> str:
    ...
    if not state.library:
        ...
        return "\n".join(lines)
    page_items, page, total_pages = paginate_items(state.library, page)
    lines.append("")
    start_idx = page * PAGE_SIZE
    for offset, item in enumerate(page_items, start=1):
        track = Track(**item)
...
def library_markup(state: ChatState, page: int = 0) -> InlineKeyboardMarkup:
    ...
    for offset, _ in enumerate(page_items):
        idx = start_idx + offset
        rows.append([
            InlineKeyboardButton("▶️", callback_data=f"lib|p|{idx}|{page}"),
    ...
    if state.panel_message_id:
        try:
            ...
            if "message is not modified" not in str(e).lower():
                logger.exception("No se pudo editar el panel fijado existente")
        except Exception:
            logger.exception("No se pudo editar el panel fijado existente")
        return
...
async def cancel_control_refresh(chat_id: int) -> None:
    task = CONTROL_REFRESH_TASKS.pop(chat_id, None)
    if task and not task.done():
        task.cancel()


async def _control_refresh_runner(chat_id: int) -> None:
    try:
        while True:
            await asyncio.sleep(15)
            state = get_state(chat_id)
            if not state.dj_mode or not state.now_playing or state.paused or not VOICE.application:
                return
            await ensure_panel(VOICE.application.bot, chat_id)
            await ensure_control_panel(VOICE.application.bot, chat_id)
    except asyncio.CancelledError:
        ...
        logger.exception("Error actualizando el panel del chat %s", chat_id)


async def ensure_control_refresh(chat_id: int) -> None:
    await cancel_control_refresh(chat_id)
    state = get_state(chat_id)
    if state.dj_mode and state.now_playing and not state.paused:
        CONTROL_REFRESH_TASKS[chat_id] = asyncio.create_task(_control_refresh_runner(chat_id))
...
class VoiceEngine:
    ...
    async def play_track(self, bot, chat_id: int, track: Track) -> None:
        ...
        state.paused = False
        state.playback_started_at = int(time.time())
        save_all_states()
        ...
        await ensure_control_refresh(chat_id)
...
async def text_message_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text == "dj plan" and update.message.reply_to_message:
        ...
        if track:
            state = get_state(update.effective_chat.id)
            if not state.dj_mode or update.effective_user.id != state.assigned_dj_id:
                await safe_delete(context.bot, update.effective_chat.id, update.message.message_id)
                return
...
async def music_message_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat or not update.message:
        return
    state = get_state(update.effective_chat.id)
    if not state.dj_mode or not state.assigned_dj_id:
        return
...
async def close_dj_session(bot, chat_id: int) -> None:
    state = get_state(chat_id)
    preserved_library = [dict(item) for item in state.library]
    preserved_lists = {name: [dict(i) for i in items] for name, items in state.saved_lists.items()}
    ...
    new_state = ChatState()
    new_state.library = preserved_library
    new_state.saved_lists = preserved_lists
    STATE_CACHE[chat_id] = new_state
    save_all_states()
    await cleanup_old_files(chat_id)
    await send_temp_message(bot, chat_id, "<b>SESIÓN DE DJ FINALIZADA</b>", ttl=15)
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "menu_search_help":
        await send_temp_message(
            context.bot,
            chat_id,
            "🎵 Pulsa para copiar <code>@VoiceShazamBot</code> y pégalo abajo seguido de tu búsqueda.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📋 Copiar @VoiceShazamBot", copy_text=CopyTextButton(text="@VoiceShazamBot "))]]),
            ttl=8,
        )
        return
