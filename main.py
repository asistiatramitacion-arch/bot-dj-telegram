...
async def is_assigned_dj(chat_id: int, user_id: int) -> bool:
    state = get_state(chat_id)
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "panel_refresh":
        if not await is_assigned_dj(chat_id, user_id):
    ...
    if data == "panel_close":
        if not await is_assigned_dj(chat_id, user_id):
            await query.answer("Solo el DJ asignado puede cerrar.", show_alert=True)
            return
    ...
    if data == "panel_scan_group":
        if not await is_assigned_dj(chat_id, user_id):
            await query.answer("Solo el DJ asignado puede rastrear el grupo.", show_alert=True)
            return
    ...
    if data == "panel_pause_resume":
        ...
        if not state.now_playing:
            if state.queue:
                ...
                return
            await query.answer("No hay ninguna canción en cola para reproducir.", show_alert=True)
            return
