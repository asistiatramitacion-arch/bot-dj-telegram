...
async def advance_to_next(chat_id: int, reason: str) -> bool:
    import time as _time

    now = _time.monotonic()
    last = NEXT_GUARD.get(chat_id, 0.0)
    if now - last < 2.0:
        logger.info("Salto a siguiente ignorado por guardia anti-duplicado en chat %s (%s)", chat_id, reason)
        return False

    state = get_state(chat_id)
    if state.paused:
        logger.info("Salto a siguiente ignorado porque está pausado en chat %s (%s)", chat_id, reason)
        return False

    if not state.now_playing:
        logger.info("Salto a siguiente ignorado porque no hay pista actual en chat %s (%s)", chat_id, reason)
        return False

    NEXT_GUARD[chat_id] = now
    logger.info("Avanzando a la siguiente pista en chat %s (%s)", chat_id, reason)
    ...
    try:
        while True:
            ...
            if remaining <= trigger:
                await advance_to_next(chat_id, "monitor")
                return
            if remaining <= 0:
                await advance_to_next(chat_id, "monitor")
                return
    ...
    async def start(self, application: Application) -> None:
        ...
        async def _on_stream_end(_, update):
            try:
                ...
                if chat_id is None:
                    return
                logger.info("Evento on_stream_end recibido en chat %s", chat_id)
                await advance_to_next(chat_id, "on_stream_end")
            except Exception:
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "panel_next":
        ...
        if not state.queue:
            ...
            return
        await advance_to_next(chat_id, "panel_next")
        return
