...
class ChatState:
    ...
    volume: int = 100
    play_started_at: int = 0
    paused_position: int = 0
...
def remaining_seconds(state: ChatState) -> int:
    ...
    if track.duration <= 0:
        return 0
    if state.paused:
        elapsed = max(0, state.paused_position)
    else:
        started = state.play_started_at or int(time.time())
        elapsed = max(0, int(time.time()) - started)
    return max(0, track.duration - elapsed)


def clamp_page(page: int, total_items: int, page_size: int = VIEW_PAGE_SIZE) -> int:
    if total_items <= 0:
        return 0
...
def paged_slice(items: List[Dict[str, Any]], page: int, page_size: int = VIEW_PAGE_SIZE):
...
def control_header(state: ChatState) -> str:
    ...
    remaining = fmt_duration(remaining_seconds(state)) if state.now_playing else "--:--"
    return (
...
def control_panel_markup(state: ChatState) -> InlineKeyboardMarkup:
    ...
    auto_label = f"⏭️ Auto-sig {state.autoplay_offset}"
...
def queue_text(state: ChatState, page: int = 0) -> str:
    ...
    page, start, end, chunk = paged_slice(state.queue, page)
    total_pages = max(1, (len(state.queue) - 1) // VIEW_PAGE_SIZE + 1)
...
def queue_markup(state: ChatState, page: int = 0) -> InlineKeyboardMarkup:
    ...
    page, start, end, chunk = paged_slice(state.queue, page)
    for rel_idx, _ in enumerate(chunk):
        idx = start + rel_idx
        row: List[InlineKeyboardButton] = [InlineKeyboardButton("▶️", callback_data=f"q|p|{idx}")]
        if idx > 0:
    ...
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️", callback_data=f"qpg|{page - 1}"))
...
def library_text(state: ChatState, page: int = 0) -> str:
    ...
    page, start, end, chunk = paged_slice(state.library, page)
    total_pages = max(1, (len(state.library) - 1) // VIEW_PAGE_SIZE + 1)
    lines.append(f"\nPágina {page + 1}/{total_pages}\n")
    for idx, item in enumerate(chunk, start=start + 1):
        track = Track(**item)
        meta: List[str] = []
...
def library_markup(state: ChatState, page: int = 0) -> InlineKeyboardMarkup:
    ...
    for rel_idx, _ in enumerate(chunk):
        idx = start + rel_idx
        rows.append([
            InlineKeyboardButton("▶️", callback_data=f"lib|p|{idx}"),
    ...
    if nav:
        rows.append(nav)
    rows.append([
        InlineKeyboardButton("▶️ Reproducir todas", callback_data=f"lib|pa|{page}"),
        InlineKeyboardButton("➕ Cola todas", callback_data=f"lib|qa|{page}"),
    ])
    rows.append([InlineKeyboardButton("🔙 Volver", callback_data="lib|r|0")])
    return InlineKeyboardMarkup(rows)


def saved_lists_text(state: ChatState, page: int = 0) -> str:
    ...
    page = clamp_page(page, len(names), VIEW_PAGE_SIZE)
    start = page * VIEW_PAGE_SIZE
    end = start + VIEW_PAGE_SIZE
    chunk = names[start:end]
    total_pages = max(1, (len(names) - 1) // VIEW_PAGE_SIZE + 1)
    lines.append(f"\nPágina {page + 1}/{total_pages}\n")
    for idx, name in enumerate(chunk, start=start + 1):
        lines.append(f"{idx}. {name} ({len(state.saved_lists[name])})")
    return "\n".join(lines)


def saved_lists_markup(state: ChatState, page: int = 0) -> InlineKeyboardMarkup:
    rows: List[List[InlineKeyboardButton]] = []
    names = sorted(state.saved_lists.keys())
    page = clamp_page(page, len(names), VIEW_PAGE_SIZE)
    start = page * VIEW_PAGE_SIZE
    ...
    chunk = names[start:end]
    for rel_idx, name in enumerate(chunk):
        idx = start + rel_idx
        rows.append([
            InlineKeyboardButton("▶️", callback_data=f"lst|p|{idx}"),
            InlineKeyboardButton("➕", callback_data=f"lst|a|{idx}"),
            InlineKeyboardButton("🗑️", callback_data=f"lst|x|{idx}"),
        ])
    nav: List[InlineKeyboardButton] = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️", callback_data=f"lstpg|{page - 1}"))
    if end < len(names):
        nav.append(InlineKeyboardButton("➡️", callback_data=f"lstpg|{page + 1}"))
    if nav:
        rows.append(nav)
...
async def ensure_panel(bot, chat_id: int) -> None:
    ...
    if state.panel_message_id:
        try:
            ...
            msg_err = str(e).lower()
            if "message to edit not found" in msg_err or "message identifier is not specified" in msg_err:
                pass
            else:
                logger.exception("No se pudo editar el panel fijado")
                return
        except Exception:
            logger.exception("No se pudo editar el panel fijado")
            return
...
async def _autoplay_monitor(chat_id: int) -> None:
    try:
        while True:
            state = get_state(chat_id)
            if not state.now_playing:
            ...
            trigger_at = max(0, -state.autoplay_offset)
            ...
            if VOICE.application:
                try:
                    ...
                    logger.exception("No se pudo refrescar el panel en autoplay de %s", chat_id)

            if state.queue and remaining <= trigger_at:
            ...
            sleep_for = min(10, max(1, remaining - trigger_at))
            await asyncio.sleep(sleep_for)
    except asyncio.CancelledError:
        return
    except Exception:
        logger.exception("Error en auto-siguiente de %s", chat_id)


async def schedule_autoplay(chat_id: int) -> None:
    await cancel_autoplay(chat_id)
    state = get_state(chat_id)
    if not state.now_playing or state.paused:
        return
    AUTOPLAY_TASKS[chat_id] = asyncio.create_task(_autoplay_monitor(chat_id))
...
class VoiceEngine:
    ...
    async def pause_resume(self, chat_id: int) -> None:
        ...
        if state.paused:
            ...
            state.paused = False
            state.play_started_at = int(time.time()) - max(0, state.paused_position)
            state.paused_position = 0
            await schedule_autoplay(chat_id)
        else:
            await self.calls.pause_stream(chat_id)
            state.paused = True
            state.paused_position = max(0, int(time.time()) - (state.play_started_at or int(time.time())))
            await cancel_autoplay(chat_id)
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "panel_load_lists":
        if not await is_controller(context, chat_id, user_id):
            await query.answer("Solo el DJ asignado puede usar el cuadro de mandos.", show_alert=True)
            return
        await render_control_view(context.bot, chat_id, saved_lists_text(state, 0), saved_lists_markup(state, 0))
        return
    ...
    if data.startswith("lstpg|"):
        if not await is_controller(context, chat_id, user_id):
            await query.answer("Solo el DJ asignado puede cargar listas.", show_alert=True)
            return
        page = int(data.split("|")[1])
        await render_control_view(context.bot, chat_id, saved_lists_text(state, page), saved_lists_markup(state, page))
        return
