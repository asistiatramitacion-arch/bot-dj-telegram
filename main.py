...
async def _autoplay_monitor(chat_id: int, track_token: str) -> None:
    import time as _time
    next_ui_refresh = 0
    try:
        while True:
            ...
            if VOICE.application and int(_time.time()) >= next_ui_refresh:
                ...
                try:
                    ...
                    logger.exception("No se pudo refrescar el panel durante autoplay")
            trigger = abs(state.autoplay_offset) if state.autoplay_offset < 0 and state.queue else 0
        ...
        logger.exception("Error en monitor de autoplay para chat %s", chat_id)
...
async def schedule_autoplay(chat_id: int, duration: int, offset: int) -> None:
    ...
    if duration <= 0 or not state.now_playing:
        return
    AUTOPLAY_TASKS[chat_id] = asyncio.create_task(_autoplay_monitor(chat_id, track_key(state.now_playing)))
...
async def close_dj_session(bot, chat_id: int) -> None:
    ...
    autoplay_offset = state.autoplay_offset
    volume = state.volume
