...
try:
    from pytgcalls.types.stream import StreamEnded as STREAM_END_EVENT
except ImportError:
    try:
        from pytgcalls.types.stream import StreamAudioEnded as STREAM_END_EVENT
    except ImportError:
    ...
    async def start(self, application: Application) -> None:
        ...
        async def _on_stream_end(_, update):
            try:
                ...
                if chat_id is None:
                    logger.info("on_stream_end sin chat_id: %s", type(update).__name__)
                    return

                if STREAM_END_EVENT is not None and not isinstance(update, STREAM_END_EVENT):
                    logger.info("Evento stream_end ignorado: %s", type(update).__name__)
                    return

                logger.info("Evento on_stream_end recibido en chat %s (%s)", chat_id, type(update).__name__)
                await simulate_panel_next(chat_id, reason=f"on_stream_end:{type(update).__name__}")
            except Exception:
                logger.exception("Fallo en on_stream_end")
