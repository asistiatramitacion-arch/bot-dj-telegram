...
# =========================================================
# V41 - REPARACIÓN ESTABLE DE SELECTORES PRIVADOS
# Base: v31 estable. No borra /data/state.json.
# Añade /resetprivado_total y /ajustes2 con selector aislado safe2|.
# =========================================================


def _safe_private_chat_title(chat_id: int) -> str:
    try:
...
def _safe_state_group_ids() -> List[int]:
    ids: List[int] = []
    for raw in STATE_CACHE.keys():
        try:
            cid = int(raw)
            if cid < 0:
                ids.append(cid)
...
def _rebuild_private_selectors_for_user(user_id: int) -> Tuple[int, int]:
    """Reconstruye DJ_PRIVATE_GROUPS y HOT_PRIVATE_GROUPS desde STATE_CACHE sin tocar state.json."""
    groups = _safe_state_group_ids()
    key = str(int(user_id))
    titles = {str(cid): _safe_private_chat_title(cid) for cid in groups}
    selected = groups[0] if groups else None

    try:
        DJ_PRIVATE_GROUPS[key] = {"selected": selected, "groups": dict(titles)}
        dj_save_private_groups()
    except Exception:
        logger.exception("No se pudo reconstruir DJ_PRIVATE_GROUPS")

    try:
        HOT_PRIVATE_GROUPS[key] = {"selected": selected, "groups": dict(titles)}
        hot_save_private_groups()
    except Exception:
        logger.exception("No se pudo reconstruir HOT_PRIVATE_GROUPS")

    return len(groups), int(selected or 0)


async def resetprivado_total_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if update.effective_chat and update.effective_chat.type != "private":
        await update.message.reply_text("Usa este comando en privado con el bot.")
        return
    if ADMIN_IDS and not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo el propietario/admin configurado puede reparar selectores privados.")
        return

    removed = []
    for path in [DJ_PRIVATE_GROUPS_PATH, HOT_PRIVATE_GROUPS_PATH]:
        try:
            if path.exists():
                path.unlink()
    ...
    try:
        DJ_PRIVATE_GROUPS.clear()
    except Exception:
        pass
    try:
        HOT_PRIVATE_GROUPS.clear()
    except Exception:
        pass

    total, selected = _rebuild_private_selectors_for_user(update.effective_user.id)
    lines = [
        "✅ <b>Selectores privados reconstruidos</b>",
        "",
        f"Grupos detectados en <code>/data/state.json</code>: <b>{total}</b>",
        f"Grupo seleccionado: <code>{selected or 'ninguno'}</code>",
        "",
        "Archivos limpiados:",
    ]
    if removed:
        lines.extend(f"• <code>{h(p)}</code>" for p in removed)
    else:
        lines.append("• No existían archivos privados antiguos; se han creado de nuevo.")
    lines.extend(["", "Ahora prueba:", "<code>/ajustes2</code>", "<code>/hotconfig</code>", "<code>/djconfig</code>"])
    await update.message.reply_html("\n".join(lines))


async def debugprivado_total_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    uid = update.effective_user.id
    groups = _safe_state_group_ids()
    lines = [
        "🧪 <b>Debug privado estable</b>",
        "",
        f"Usuario: <code>{uid}</code>",
        f"STATE_PATH: <code>{h(STATE_PATH)}</code> existe: <b>{STATE_PATH.exists()}</b>",
        f"DJ_PRIVATE_GROUPS_PATH: <code>{h(DJ_PRIVATE_GROUPS_PATH)}</code> existe: <b>{DJ_PRIVATE_GROUPS_PATH.exists()}</b>",
        f"HOT_PRIVATE_GROUPS_PATH: <code>{h(HOT_PRIVATE_GROUPS_PATH)}</code> existe: <b>{HOT_PRIVATE_GROUPS_PATH.exists()}</b>",
        "",
        f"Grupos en state.json: <b>{len(groups)}</b>",
    ]
    for cid in groups[:30]:
        lines.append(f"• <code>{cid}</code>: {h(_safe_private_chat_title(cid))}")
    if len(groups) > 30:
        lines.append(f"… y {len(groups)-30} más")
    lines.extend(["", f"DJ para ti: <code>{h(json.dumps(DJ_PRIVATE_GROUPS.get(str(uid), {}), ensure_ascii=False)[:1200])}</code>", f"HOT para ti: <code>{h(json.dumps(HOT_PRIVATE_GROUPS.get(str(uid), {}), ensure_ascii=False)[:1200])}</code>"])
    await update.message.reply_html("\n".join(lines))


def ajustes2_group_selector_markup() -> InlineKeyboardMarkup:
    rows: List[List[InlineKeyboardButton]] = []
    for chat_id in _safe_state_group_ids():
        title = _safe_private_chat_title(chat_id)
        rows.append([InlineKeyboardButton(f"⚙️ {title}", callback_data=f"safe2|open|{chat_id}")])
    rows.append([InlineKeyboardButton("🔧 Reconstruir selectores", callback_data="safe2|rebuild")])
    return InlineKeyboardMarkup(rows)


async def ajustes2_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if update.effective_chat and update.effective_chat.type != "private":
        await update.message.reply_text("Usa /ajustes2 en privado con el bot.")
        return
    if ADMIN_IDS and not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo administradores configurados.")
        return
    if not _safe_state_group_ids():
        await update.message.reply_html("No encuentro grupos en <code>/data/state.json</code>. Entra en el grupo y ejecuta <code>/plan</code> primero.")
        return
    await update.message.reply_html("<b>⚙️ Ajustes privados seguros</b>\n\nElige el grupo que quieres configurar:", reply_markup=ajustes2_group_selector_markup())


async def safe2_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query or not update.effective_user:
        return
    try:
    ...
    if ADMIN_IDS and not is_global_admin_user(update.effective_user.id):
        try:
            await query.message.reply_text("Solo administradores configurados.")
        except Exception:
            pass
        return
    if data == "safe2|rebuild":
        total, selected = _rebuild_private_selectors_for_user(update.effective_user.id)
        await query.message.reply_html(f"✅ Selectores reconstruidos. Grupos: <b>{total}</b>. Seleccionado: <code>{selected or 'ninguno'}</code>\n\nElige grupo:", reply_markup=ajustes2_group_selector_markup())
        return
    if data.startswith("safe2|open|"):
        try:
        ...
        try:
            ...
            hot_link_private_group(update.effective_user.id, chat_id, title)
        except Exception:
            logger.exception("No se pudo vincular grupo safe2")
        try:
            await query.message.reply_html(admin_private_main_text(chat_id), reply_markup=admin_private_main_markup(chat_id))
        except Exception:
            logger.exception("No se pudo abrir panel seguro para chat %s", chat_id)
            await query.message.reply_text("No he podido abrir el panel. Mira los logs de Railway para el traceback.")
        return
...
def build_application() -> Application:
    ...
    application.add_handler(command_handler(["resetprivado_total", "resetselectores_total"], resetprivado_total_command))
    application.add_handler(command_handler(["debugprivado_total", "debugselectores_total"], debugprivado_total_command))
    application.add_handler(command_handler("ajustes2", ajustes2_command))
    ...
    application.add_handler(CallbackQueryHandler(safe2_callback_router, pattern="^safe2\\|"))
