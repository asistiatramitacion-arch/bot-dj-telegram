...
# =========================
# V27 DJ: aviso solo con like + mesa estable salvo minimizar
# =========================
# Ajustes solicitados:
# - El aviso "EN DIRECTO AHORA" solo muestra botón ❤️ Me gusta.
# - La mesa NO se borra ni se minimiza por refrescos, cambios de canción o botones.
# - Solo desaparece si el usuario pulsa explícitamente ➖ Minimizar mesa.
...
async def ensure_control_panel(bot, chat_id: int) -> None:
    ...
    # Solo el botón explícito de minimizar activa este estado.
    if bool(cfg.get("dj_mesa_minimized", False)):
        return

    # Si no está minimizada, la mesa debe estar visible: editar si existe o crear si falta.
    ...
    try:
        ...
        # Fallback por si cambia el orden de parches en futuras versiones.
        await _old_ensure_control_panel_v23(bot, chat_id)
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if q and update.effective_chat and update.effective_user:
        ...
        # Mostrar mesa: explícitamente visible.
        if data in ("panel_home", "menu_panel"):
            if not await is_controller(context, chat_id, user_id):
                await q.answer("Solo el DJ asignado o admin puede mostrar la mesa.", show_alert=True)
                return
        ...
        # Único sitio donde se permite ocultar la mesa.
        if data == "panel_minimize":
            if not await is_controller(context, chat_id, user_id):
                await q.answer("Solo el DJ asignado o admin puede minimizar la mesa.", show_alert=True)
                return
        ...
        # Cualquier otro botón de la mesa NO debe minimizarla ni ocultarla.
...
# /djmesa fuerza mesa visible, sin tocar música ni cola.
...
# =========================
# V28 DJ MESA COMO MENSAJE ESTABLE: EDITAR, NO BORRAR/RECREAR
# =========================
# Objetivo:
# - La mesa/cuadro de mandos se crea una vez y después se EDITA siempre.
# - El panel fijado superior se edita siempre y NO toca la mesa.
# - Cambiar canción, refrescar, añadir cola, abrir lista, etc. NO debe borrar la mesa.
# - Solo se crea mesa nueva si no hay message_id guardado o Telegram confirma que no existe.
# - Solo se oculta si se pulsa explícitamente ➖ Minimizar mesa.
...
async def _edit_control_panel_v28(bot, chat_id: int, *, force_create: bool = True) -> None:
    ...
    if bool(cfg.get("dj_mesa_minimized", False)):
        # Minimizada manualmente: no tocar, no recrear.
        return
    ...
    if state.control_message_id:
        try:
            ...
            if _is_msg_not_found_error_v28(e):
                # El mensaje ya no existe. Solo aquí permitimos crear otro.
                state.control_message_id = None
                save_all_states()
            else:
                # Error temporal/HTML/etc. No crear duplicados.
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if q and update.effective_chat and update.effective_user:
        ...
        # Si el botón pertenece a la mesa, no permitir que ningún router antiguo
        # marque la mesa como oculta. Después de la acción, si sigue visible,
        # se editará el mismo mensaje.
        if data.startswith(("panel_", "q|", "lib|", "lst|")) and data not in ("panel_join_live", "panel_voice_info"):
...
# =========================
# V29 DJ: LIVE abre directo + aviso canción independiente funcionando
# =========================
# Ajustes:
# - panel_join_live intenta iniciar/asegurar videochat y abrir enlace.
# - panel_live_toggle, al activar LIVE, intenta abrir el enlace directamente; si no hay URL,
#   deja un mensaje con instrucciones/botón cuando sea posible.
# - panel_announce_now se intercepta al final de la cadena para que siempre funcione.
# - Al activar aviso canción, si ya hay una canción sonando, manda el aviso inmediatamente.
# - El aviso solo lleva ❤️ Me gusta.
...
async def _toggle_dj_announce_now_v29(context: ContextTypes.DEFAULT_TYPE, query, chat_id: int) -> None:
    ...
    # Refrescar mesa sin tocar/minimizar.
    try:
        if not bool(cfg.get("dj_mesa_minimized", False)):
            await ensure_control_panel(context.bot, chat_id)
    except Exception:
    ...
    # Si se activa con una canción ya sonando, mandar aviso inmediatamente.
    if enabled:
        try:
            state = get_state(chat_id)
            if state.now_playing:
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if q and update.effective_chat and update.effective_user:
        ...
        # Botón de aviso canción: independiente del panel fijado superior.
        ...
        # Ir directo: intenta iniciar/asegurar el videochat y abrirlo.
        if data in ("panel_join_live", "panel_voice_info"):
        ...
        # LIVE ON/OFF: al activar, arranca videochat y abre enlace si existe.
        if data in ("panel_live_toggle", "panel_pause_resume"):
            if not await is_controller(context, chat_id, user_id):
            ...
            try:
                live_result = await VOICE.toggle_live(chat_id)
                # Limpieza conservadora: no tocar mesa si está visible.
                try:
                    await cleanup_bot_messages_keep_core(context.bot, chat_id)
                except Exception:
                    pass
...
# =========================================================
# V30 - Pregonero auto anti-duplicados, validación reforzada,
# previews HTML renderizadas y frase del día con siguiente.
# Mantiene /data/state.json y no resetea configuraciones.
# =========================================================

# --- Helpers de fecha/hora local reutilizables ---
...
# --- Pregonero automático: marcar antes de enviar para que no se repita varias veces. ---
    ...
    while True:
        ...
        for chat_id in list(STATE_CACHE.keys()):
            try:
                cfg = hot_cfg(chat_id)
                jobs = pregonero_auto_jobs(chat_id)
                # Migración blanda de configuraciones antiguas si no hay jobs.
                if bool(cfg.get("pregonero_auto_enabled", False)) and not jobs:
                    sched = cfg.get("pregonero_auto_schedule") if isinstance(cfg.get("pregonero_auto_schedule"), dict) else None
                    if not sched:
                        hhmm = str(cfg.get("pregonero_auto_time", "21:00"))
                ...
                for job in list(jobs):
                    ...
                    # Bloqueo y marca ANTES de enviar para evitar duplicado por tick/reinicio rápido.
...
# --- Validación reforzada: cualquier entrada detectada entra en validación. ---
async def validation_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat or not update.message:
        return
    chat_id = int(update.effective_chat.id)
...
async def validation_chat_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Autoaprobación OFF: queda pendiente de botón admin, pero al aprobar se validará sí o sí.
...
async def validation_chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Primero ejecuta despedidas/salidas de versiones anteriores si existen.
    try:
    ...
    # Entrada: left/kicked/no-status -> member/restricted/administrator/creator.
    ...
    # Salida/expulsión: mantener aviso de despedida aunque sea otro bot/admin.
    if old_status not in ("left", "kicked", "") and new_status in ("left", "kicked"):
        try:
...
# --- Vistas previas: enviar renderizado real, no código HTML en <pre>. ---
async def send_profile_preview(bot, private_chat_id: int, target_chat_id: int, profile: str) -> None:
    title = MESSAGE_PROFILES.get(profile, MESSAGE_PROFILES["welcome"])["title"]
    text = fake_profile_text(target_chat_id, profile)
    if profile == "pregonero":
...
# --- Frase del día: permitir pasar a otra frase si la actual no gusta. ---
...
# =========================
# PATCH V31: Pregonero recurrente configurable por perfil + salidas voluntarias
# =========================
...
# Sobrescribimos due para que el pregonero no salga antes/después: solo minuto exacto.
def pregonero_auto_due(cfg: Dict[str, Any], now_ts: int) -> bool:
    if not bool(cfg.get("pregonero_auto_enabled", cfg.get("enabled", True))):
        return False
    sched = cfg.get("pregonero_auto_schedule") if isinstance(cfg.get("pregonero_auto_schedule"), dict) else cfg.get("schedule") if isinstance(cfg.get("schedule"), dict) else None
...
# Usuarios que se fueron voluntariamente / no expulsados.
...
# Compatibilidad: al crear un job nuevo captura perfil actual y nombre limpio.
...
# Admin UI: listas + pregonero con edición por job.
...
def admin_private_block_markup(chat_id: int, block: str, page: int = 0) -> InlineKeyboardMarkup:
    ...
    if block == "pregonero":
        ...
        for job in jobs[:10]:
            ...
            rows.append([InlineKeyboardButton(f"{status} #{jid} {label}", callback_data=f"cfg|pauto_edit|{chat_id}|{jid}|pregonero")])
            rows.append([
                InlineKeyboardButton("👁", callback_data=f"cfg|pauto_preview|{chat_id}|{jid}|pregonero"),
                InlineKeyboardButton("ON/OFF", callback_data=f"cfg|pauto_toggle|{chat_id}|{jid}|pregonero"),
                InlineKeyboardButton("🚀", callback_data=f"cfg|pauto_send|{chat_id}|{jid}|pregonero"),
                InlineKeyboardButton("🗑", callback_data=f"cfg|pauto_delete|{chat_id}|{jid}|pregonero"),
            ])
        rows.extend(block_footer_rows(chat_id, block))
        return InlineKeyboardMarkup(rows)
    return _old_admin_private_block_markup_v31(chat_id, block, page)
...
async def admin_private_config_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if q and data.startswith("cfg|"):
        ...
        try:
            ...
            chat_id = 0
        # Listado de usuarios que se fueron.
        if action == "leftusers":
            ...
            return
        # Ficha de cada pregonero automático.
        if action.startswith("pauto_") and len(parts) >= 4:
            ...
            if action in ("pauto_name", "pauto_text", "pauto_buttons", "pauto_media", "pauto_schedule"):
                ...
                return
        # Mantener compatibilidad con los botones viejos de borrar/toggle.
...
async def validation_chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Registrar salidas voluntarias antes de ejecutar la lógica original.
    try:
        ...
        # Telegram suele marcar salida voluntaria como left; expulsión como kicked.
...
# =========================
# PATCH V32: Encuestas anónimas con resultados ocultos hasta finalizar
# =========================
...
def _v32_parse_duration_seconds(value: str, default: int = 3600) -> int:
    ...
    if not raw:
        return default
    # admite 30m, 1h, 2d, 45s o minutos sueltos.
    try:
        if raw.endswith("s"):
...
def _v32_parse_poll_spec(text: str) -> Tuple[str, List[str], int, bool]:
    ...
    close_seconds = 3600
    # Si la última parte parece duración, úsala.
...
# Hacer visible el bloque de encuestas en /ajustes sin tocar la estructura anterior.
try:
    if ("polls", "📊 Encuestas") not in ADMIN_PRIVATE_BLOCKS:
        # Antes de Listados y control, si existe.
...
def build_application() -> Application:
    ...
    # Añadimos comandos específicos. El router genérico ya intercepta callbacks poll|.
...
# =========================
# PATCH V33: Encuestas avanzadas + avisos privados + ping de estado
# =========================
...
def _v32_duration_label(seconds: int) -> str:  # type: ignore[override]
    return _v33_duration_label(seconds)


def _v32_parse_duration_seconds(value: str, default: int = 3600) -> int:  # type: ignore[override]
...
def _v32_poll_text(poll: Dict[str, Any], *, results: bool = False) -> str:  # type: ignore[override]
    q = h(poll.get("question", "Encuesta"))
...
def _v32_poll_markup(chat_id: int, poll: Dict[str, Any]) -> Optional[InlineKeyboardMarkup]:  # type: ignore[override]
    if str(poll.get("status") or "open") != "open":
        return None
    poll_id = int(poll.get("id", 0) or 0)
    opts = poll.get("options") if isinstance(poll.get("options"), list) else []
...
async def _v32_create_poll(context: ContextTypes.DEFAULT_TYPE, chat_id: int, question: str, options: List[str], close_seconds: int, multi: bool, creator_id: int = 0) -> Dict[str, Any]:  # type: ignore[override]
    poll_id = _v32_next_poll_id(chat_id)
    now = _v33_now_ts()
...
async def _v32_close_poll(context: ContextTypes.DEFAULT_TYPE, chat_id: int, poll: Dict[str, Any], *, silent: bool = False) -> None:  # type: ignore[override]
    if str(poll.get("status") or "open") != "open":
        return
    poll["status"] = "closed"
...
async def poll_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    q = update.callback_query
    if not q:
        return
    data = q.data or ""
...
async def poll_close_loop(context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    now = _v33_now_ts()
    for chat_id, state in list(STATE_CACHE.items()):
        try:
            for poll in list(_v32_polls(chat_id)):
...
def admin_private_block_markup(chat_id: int, block: str, page: int = 0) -> InlineKeyboardMarkup:  # type: ignore[override]
    if block == "polls":
        ...
        rows.extend(block_footer_rows(chat_id, block))
        return InlineKeyboardMarkup(rows)
    return _old_admin_private_block_markup_v33(chat_id, block, page)
...
async def admin_private_config_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:  # type: ignore[override]
    ...
    if kind == "cfg_poll_wizard_duration":
        ...
        poll = {"id": _v32_next_poll_id(chat_id), "question": draft.get("question", "Encuesta"), "description": draft.get("description", ""), "options": draft.get("options", []), "multi": False, "anonymous": True, "status": "draft", "creator_id": int(update.effective_user.id), "created_ts": _v33_now_ts(), "duration_seconds": duration, "close_ts": 0, "message_id": 0, "votes": {}, "media": None, "buttons": [], "pin_notice": False, "repeat_enabled": False, "repeat_seconds": 0, "next_repeat_ts": 0}
...
async def admin_private_config_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    if not update.effective_user or not update.message or not update.effective_chat or update.effective_chat.type != "private":
...
async def validation_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
...
async def farewell_left_member(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
...
async def validation_chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    try:
        cmu = update.chat_member
        if cmu:
...
async def mute_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
...
async def ping_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    if update.effective_chat and update.effective_chat.type == "private":
...
def build_application() -> Application:  # type: ignore[override]
    ...
    # Handler extra para avisar al dueño cuando lo mencionan.
...
# =========================
# PATCH V42: Encuestas más limpias + Juego del Impostor
# =========================
...
# Reemplaza la vista pública de encuesta: sin botón de finalizar abajo; muestra tiempo restante y votos.
def _v32_poll_text(poll: Dict[str, Any], *, results: bool = False) -> str:  # type: ignore[override]
    q = h(poll.get("question", "Encuesta"))
    desc = str(poll.get("description") or "").strip()
    multi = bool(poll.get("multi", False))
...
def _v32_poll_markup(chat_id: int, poll: Dict[str, Any]) -> Optional[InlineKeyboardMarkup]:  # type: ignore[override]
    ...
    for idx, opt in enumerate(opts):
        rows.append([InlineKeyboardButton(f"{idx+1}. {str(opt)[:48]}", callback_data=f"poll|vote|{chat_id}|{poll_id}|{idx}")])
    # Botón informativo: Telegram exige callback_data; al pulsarlo solo muestra aviso.
    ...
    for chat_id, state in list(STATE_CACHE.items()):
        try:
            for poll in list(_v32_polls(chat_id)):
                ...
                # Refresca el tiempo restante una vez por minuto como máximo.
                if now - last >= 60:
...
async def impostor_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action in ("next", "show_turn"):
        ...
        if action == "next":
            # Puede avanzar el jugador del turno o un admin.
...
# =========================
# PATCH V43: Impostor fluido, recientes DJ privado robustos, alertas privadas
# =========================

# --- DJ privado: guardar recientes por usuario y grupo destino, no solo por chat privado.
...
def _dj_private_recent_tracks(chat_id: int, user_id: int, *, seconds: int = 1800, limit: int = 100) -> List[Track]:
    ...
    if isinstance(user_bucket, list):
        for raw in user_bucket:
            ...
            if tr:
                rows.append((ts, tr))
    # También incluye recientes detectadas directamente en el grupo, por si las descargas llegaron allí.
    try:
...
def _remember_dj_private_recent(chat_id: int, user_id: int, track: Track) -> None:
    ...
    data["_detected_by"] = int(user_id)
    bucket.append(data)
    # conserva solo últimas 120 por usuario
    bucket = bucket[-120:]
...
async def dj_private_music_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    if not update.effective_chat or not update.effective_user or not update.message:
        return
    if update.effective_chat.type != "private":
        return
...
async def djprivate_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    query = update.callback_query
    data = query.data if query else ""
    if query and data.startswith("djpriv|recentall|"):
        try:
...
# --- Juego del Impostor más fluido: turnos de palabra, acusación inmediata y eliminación por fallo.
...
def _imp_vote_markup(chat_id: int, game: Dict[str, Any]) -> InlineKeyboardMarkup:  # type: ignore[override]
    gid = int(game.get("id", 0) or 0)
    rows: List[List[InlineKeyboardButton]] = []
    row: List[InlineKeyboardButton] = []
    for p in _imp_active_players(game):
...
async def impostor_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    ...
    if action == "start":
        # Dejamos que la versión base arranque, pero inicializamos campos nuevos después.
    ...
    # Para join/leave/cancel/secret/vote dejamos la base, pero con nuestras funciones override de markup/texto.
    return await _old_impostor_callback_router_v43(update, context)


# --- Alertas privadas robustas a propietarios/admin raíz.
...
async def _send_owner_private_alert(context: ContextTypes.DEFAULT_TYPE, text: str, *, url: Optional[str] = None) -> None:
    ...
    for uid in _owner_alert_targets():
        try:
            ...
            # Telegram bloquea privado si el dueño no abrió chat con el bot. No rompemos flujo.
            pass
...
async def owner_mention_alert_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    ...
    if not text:
        return
    # Detecta menciones por @username del propio dueño/admin si Telegram lo permite resolver desde ADMIN_IDS no siempre tenemos username.
    targets = _owner_alert_targets()
    mentioned = False
    # Si el mensaje responde al dueño/admin también avisa.
    if update.message.reply_to_message and update.message.reply_to_message.from_user:
        if int(update.message.reply_to_message.from_user.id) in targets:
            mentioned = True
    # Menciones explícitas por text_mention.
    try:
...
def build_application() -> Application:  # type: ignore[override]
    ...
    # Elimina/evita duplicados no es trivial; añadimos handlers en grupos tempranos para alertas y routers override.
    ...
    # Asegura que el router de impostor nuevo capture antes que el viejo.
    app.add_handler(CallbackQueryHandler(impostor_callback_router, pattern="^imp\\|"), group=-55)
    # Asegura que DJ privados recientes se procese con el router nuevo antes que callbacks antiguos.
...
# =========================
# PATCH V44: Impostor ranking/instrucciones, DJ recientes sin duplicar, alertas privadas configurables
# =========================
...
async def impostor_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    ...
    if action == "vote" and len(parts) >= 5:
        # Dejamos que el router base registre voto; _imp_finish calculará ganador al cerrar.
        return await _old_impostor_callback_router_v44(update, context)

    return await _old_impostor_callback_router_v44(update, context)


# --- DJ privado: añadir recientes a cola sin duplicar cola.
...
# --- Alertas privadas configurables por tipo desde /ajustes.
ALERT_PREFS = [
...
# Desactiva el emisor viejo sin tipo, para que las alertas dependan del menú nuevo.
async def _send_owner_private_alert(context: ContextTypes.DEFAULT_TYPE, text: str, *, url: Optional[str] = None) -> None:  # type: ignore[override]
    return
...
def build_application() -> Application:  # type: ignore[override]
    ...
    # Handlers priorizados para las nuevas funciones. Los emisores antiguos quedan anulados por _send_owner_private_alert no-op.
...
# =========================================================
# V45 - MÓDULO IA PLANECITO
# - /planecito pregunta a la IA con límite diario por usuario.
# - /tiempo ciudad consulta el tiempo actual.
# - Respuestas automáticas IA configurables por grupo.
# - Buenos días IA programado, con imagen local generada si Pillow está disponible.
# - Configuración privada por botones en /ajustes -> 🤖 IA Planecito.
# Mantiene rutas /data y no resetea configuraciones.
# =========================================================
...
if ("ai", "🤖 IA Planecito") not in ADMIN_PRIVATE_BLOCKS:
    try:
        # Lo colocamos antes de Listados y control si existe.
...
def admin_private_block_text(chat_id: int, block: str, page: int = 0) -> str:  # type: ignore[override]
    ...
    lines = [
        ...
        f"Grupo: <b>{h(title)}</b>",
        "",
        f"IA: <b>{_ai_bool_label(bool(cfg_value(chat_id, 'ai_enabled', True)))}</b>",
        f"Respuestas automáticas: <b>{_ai_bool_label(bool(cfg_value(chat_id, 'ai_auto_enabled', False)))}</b>",
        f"Frecuencia auto: <b>{int(cfg_value(chat_id, 'ai_auto_frequency', 10) or 10)}%</b>",
        f"Tono: <b>{h(cfg_value(chat_id, 'ai_tone', 'divertido'))}</b>",
        f"Límite /planecito por usuario/día: <b>{_ai_limit_label(cfg_value(chat_id, 'ai_planecito_daily_limit', 5))}</b>",
        f"Límite auto por usuario/día: <b>{_ai_limit_label(cfg_value(chat_id, 'ai_auto_daily_limit', 3))}</b>",
        f"Modelo: <b>{h(cfg_value(chat_id, 'ai_model', AI_DEFAULT_MODEL))}</b>",
        f"OPENAI_API_KEY: <b>{api_ok}</b>",
        "",
        f"Buenos días IA: <b>{_ai_bool_label(bool(cfg_value(chat_id, 'ai_good_morning_enabled', False)))}</b>",
        f"Hora: <b>{h(cfg_value(chat_id, 'ai_good_morning_time', '09:00'))}</b>",
        f"Imagen: <b>{_ai_bool_label(bool(cfg_value(chat_id, 'ai_good_morning_image', False)))}</b>",
        "",
        "Comandos:",
        "<code>/planecito pregunta</code>",
        "<code>/tiempo ciudad</code>",
    ]
    return "\n".join(lines)
...
# =========================================================
# V46 - IMPOSTOR RESPUESTA VISUAL + ALERTAS AJUSTADAS + /CEREBRITO
# =========================================================

# Renombra visualmente el bloque de IA sin tocar claves internas.
try:
    ADMIN_PRIVATE_BLOCKS[:] = [(k, ("🧠 IA Cerebrito" if k == "ai" else label)) for k, label in ADMIN_PRIVATE_BLOCKS]
except Exception:
    pass

# Preferencia nueva para avisos por etiquetas/hashtags especiales.
try:
...
def _v51_ai_text(chat_id: int) -> str:
    ...
    return "\n".join([
        ...
        f"Grupo: <b>{h(_v51_title(chat_id))}</b>",
        "",
        f"IA: <b>{_v51_bool_label(cfg_value(chat_id, 'ai_enabled', True))}</b>",
        f"Auto respuestas: <b>{_v51_bool_label(cfg_value(chat_id, 'ai_auto_enabled', False))}</b>",
        f"Frecuencia auto: <b>{h(cfg_value(chat_id, 'ai_auto_frequency', 10))}%</b>",
        f"Tono: <b>{h(cfg_value(chat_id, 'ai_tone', 'divertido'))}</b>",
        f"Límite /cerebrito por usuario/día: <b>{h(_v51_ai_limit(cfg_value(chat_id, 'ai_planecito_daily_limit', 5)))}</b>",
        f"OPENAI_API_KEY: <b>{api_ok}</b>",
        "",
        f"Buenos días IA: <b>{_v51_bool_label(cfg_value(chat_id, 'ai_good_morning_enabled', False))}</b>",
        f"Hora: <b>{h(cfg_value(chat_id, 'ai_good_morning_time', '09:00'))}</b>",
        f"Imagen: <b>{_v51_bool_label(cfg_value(chat_id, 'ai_good_morning_image', False))}</b>",
        "",
        f"Comandos IA personalizados: <b>{count}</b>",
        "Comando principal: <code>/cerebrito pregunta</code>",
    ])
