...
def load_all_states() -> None:
    ...
    for chat_id_str, state_data in raw.items():
        try:
            ...
            state.library = dedupe_track_items(state.library)
...
async def scan_group_history_for_tracks(chat_id: int, limit: int = SCAN_LIMIT) -> tuple[int, int]:
    ...
    found = 0
    added = 0
    existing_keys = {track_fingerprint_from_dict(item) for item in state.library}
    ...
    async for message in VOICE.client.iter_messages(chat_id, limit=limit):
        try:
            ...
            if not track:
                continue
            found += 1
            unique_key = track_fingerprint(track)
    ...
    try:
        while True:
            ...
            if remaining <= trigger:
                await simulate_panel_next(chat_id, reason="auto_next_trigger")
                return
            if remaining <= 0:
                await simulate_panel_next(chat_id, reason="auto_next_end")
                return
...
async def add_to_library(chat_id: int, track: Track) -> bool:
    ...
    existing = {track_fingerprint_from_dict(item) for item in state.library}
    if fp in existing:
        return False
    state.library.append(asdict(track))
    state.library = dedupe_track_items(state.library)
...
async def close_dj_session(bot, chat_id: int) -> None:
    ...
    library = dedupe_track_items(state.library)
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "panel_pause_resume":
        ...
        try:
            ...
            logger.exception("Fallo al ejecutar play/pausa en chat %s", chat_id)
            await query.answer("Error al ejecutar play/pausa.", show_alert=True)
        return
    ...
    if data == "panel_next":
        ...
        if not ok:
            await query.answer("No se pudo pasar a la siguiente.", show_alert=True)
        return
    ...
    if data == "panel_prev":
        ...
        if not state.history:
            ...
            return
        ok = await simulate_panel_prev(context.bot, chat_id, reason="manual_button")
        if not ok:
            await query.answer("No se pudo volver a la anterior.", show_alert=True)
        return
