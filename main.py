...
def admin_private_block_text(chat_id: int, block: str) -> str:
    ...
    if block in PROFILE_BLOCK_IDS:
        ...
        if block == "pregonero":
            ...
            psched = cfgp.get("pregonero_auto_schedule") if isinstance(cfgp.get("pregonero_auto_schedule"), dict) else None
            auto_label = psched.get("label", "") if psched else (f"diario {cfgp.get('pregonero_auto_time', '21:00')}" if cfgp.get("pregonero_auto_enabled") else "OFF")
            ...
            lines.append(f"Auto: <b>{'ON' if cfgp.get('pregonero_auto_enabled') else 'OFF'}</b> · {h(auto_label)}")
            lines.append("")
...
def admin_private_block_markup(chat_id: int, block: str) -> InlineKeyboardMarkup:
    ...
    if block == "recurrentes":
        ...
        rows.append([
            InlineKeyboardButton("📅 Nuevo semanal", callback_data=f"cfg|rec_new_custom|{chat_id}|recurrentes"),
            InlineKeyboardButton("⏱ Otro horario", callback_data=f"cfg|rec_new_custom|{chat_id}|recurrentes"),
    ...
    if block in PROFILE_BLOCK_IDS:
        ...
        if block == "pregonero":
            ...
            rows.append([
                InlineKeyboardButton("⏰ Auto pregonero", callback_data=f"cfg|pregonero_auto_set|{chat_id}|pregonero"),
                InlineKeyboardButton("🛑 Auto OFF", callback_data=f"cfg|pregonero_auto_off|{chat_id}|pregonero"),
            ])
        rows.extend(block_footer_rows(chat_id, block))
        return InlineKeyboardMarkup(rows)
...
async def admin_private_config_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "toggle_validation":
        ...
        await query.edit_message_text(
            ...
            "Para desactivar escribe <code>off</code>.",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancelar", callback_data=f"cfg|block|{chat_id}|pregonero")]])
        )
        return
        ...
        save_all_states()
        await query.answer("Auto pregonero desactivado ✅")
...
async def admin_private_config_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if kind == "cfg_pregonero_auto":
        ...
        if text.strip().lower() in ("off", "desactivar", "no", "quitar"):
            ...
            cfgp.pop("pregonero_auto_schedule", None)
            save_all_states(); pop_config_pending(update.effective_user.id)
            await reply_current_panel("✅ Pregonero automático desactivado.")
            return True
        try:
        ...
        cfgp.pop("pregonero_auto_last_day", None)
        save_all_states(); pop_config_pending(update.effective_user.id)
        await reply_current_panel(f"✅ Pregonero automático activado: <b>{h(str(sched.get('label', '')))}</b>")
        return True
    if kind == "cfg_text":
