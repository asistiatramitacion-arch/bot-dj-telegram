...
HOT_PRIVATE_GROUPS: Dict[str, int] = {}
...
def hot_load_private_groups() -> None:
    ...
    try:
        if HOT_PRIVATE_GROUPS_PATH.exists():
            data = json.loads(HOT_PRIVATE_GROUPS_PATH.read_text(encoding="utf-8"))
            HOT_PRIVATE_GROUPS = {str(k): int(v) for k, v in data.items()} if isinstance(data, dict) else {}
    except Exception:
...
def hot_link_private_group(user_id: int, chat_id: int) -> None:
    HOT_PRIVATE_GROUPS[str(int(user_id))] = int(chat_id)
    hot_save_private_groups()


def hot_private_group_for(user_id: int) -> Optional[int]:
    try:
        return int(HOT_PRIVATE_GROUPS.get(str(int(user_id))))
    except Exception:
        return None
...
def hot_cfg(chat_id: int) -> Dict[str, Any]:
    ...
    cfg.setdefault("hot_auto_activity_window_seconds", 180)
...
def hot_pick_level(chat_id: int, automatic: bool = False) -> int:
    ...
    if automatic:
        levels = [1, 2]
        if bool(cfg.get("hot_auto_include_hot", False)):
            levels.append(3)
        return int(secrets.choice(levels))
    if mode == "random":
...
def hot_auto_has_enough_interaction(chat_id: int, now: Optional[int] = None) -> bool:
    ...
    min_users = max(1, int(cfg.get("hot_auto_min_users", 2) or 2))
    cutoff = now - window
...
def hot_auto_activity_label(chat_id: int) -> str:
    cfg = hot_cfg(chat_id)
    return f"{int(cfg.get('hot_auto_min_messages', 3) or 3)} msgs / {int(cfg.get('hot_auto_min_users', 2) or 2)} users"


def hot_register_question(chat_id: int, message_id: int, target_user, level: int = 1) -> None:
...
def hot_config_text(chat_id: int) -> str:
    ...
    return (
        ...
        f"Auto puede meter HOT nivel 3: <b>{bool_label(cfg.get('hot_auto_include_hot', False))}</b>\n"
        ...
        f"Preguntas añadidas por el grupo: <b>{custom_total}</b>\n\n"
        "Comandos:\n"
...
def hot_config_markup(chat_id: int) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton("Auto " + ("ON ✅" if auto else "OFF ❌"), callback_data="hot|toggle|auto"),
         InlineKeyboardButton("Auto HOT " + ("ON" if includehot else "OFF"), callback_data="hot|toggle|autohot")],
...
async def hotconfig_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat_id is None:
        await update.message.reply_html("Primero entra en el grupo y ejecuta <code>/hotgrupo</code>. Luego vuelve aquí y usa <code>/hotconfig</code> por privado.")
        return
    if not await is_admin(context, chat_id, update.effective_user.id):
...
async def hotgrupo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await is_admin(context, chat_id, update.effective_user.id):
        await update.message.reply_text("Solo administradores.")
        return
    hot_link_private_group(update.effective_user.id, chat_id)
    await hot_delete_command_if_configured(update, context, chat_id)
    msg = await update.message.reply_html("✅ Grupo vinculado para configurar Preguntita por privado. Ábreme por privado y escribe <code>/hotconfig</code>.")
    await register_bot_message(chat_id, msg.message_id)
...
async def addmasivo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await is_admin(context, chat_id, update.effective_user.id):
        ...
        return
    await hot_delete_command_if_configured(update, context, chat_id)
    HOT_PENDING_ADD[update.effective_user.id] = {"target_chat_id": chat_id, "input_chat_id": int(update.effective_chat.id), "mode": "bulk_wait_text", "questions": []}
...
async def hotadd_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not questions:
        await query.answer("No hay preguntas válidas.", show_alert=True)
        return
    ...
    await query.message.reply_html(f"✅ Añadidas <b>{len(questions)}</b> preguntas al nivel <b>{level}</b>.")
...
async def hot_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if pending and pending.get("mode") == "bulk_wait_text" and int(pending.get("input_chat_id", chat_id)) == chat_id:
        ...
        if await is_admin(context, target_chat_id, user.id):
            ...
            rows = [[InlineKeyboardButton(f"Nivel {i}", callback_data=f"hotadd|{i}") for i in (1, 2, 3)], [InlineKeyboardButton("Nivel 4", callback_data="hotadd|4"), InlineKeyboardButton("Nivel 5", callback_data="hotadd|5")]]
            await update.message.reply_html(f"📊 Detectadas <b>{len(lines)}</b> preguntas. Elige nivel:", reply_markup=InlineKeyboardMarkup(rows))
            return
    ...
    msg = await update.message.reply_html(f"🔥 <b>{h(display_name(user))}</b> +{points} pts <i>(nivel {level}{extra})</i>\n🏆 Total: <b>{total}</b>")
    await register_bot_message(chat_id, msg.message_id)
    ...
    while True:
        ...
        for chat_id in list(STATE_CACHE.keys()):
            try:
                ...
                text = f"🎯 {mention}\n\n🤖 <b>Preguntita automática · Nivel {level}</b>\n\n💬 {h(question)}"
                ...
                fake.last_name = ""
                fake.username = None
                hot_register_question(chat_id, msg.message_id, fake, level)
...
async def on_startup(application: Application) -> None:
    global VALIDATION_WATCHDOG_TASK, HOT_AUTO_TASK
