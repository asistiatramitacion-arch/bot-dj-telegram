...
def admin_private_main_text(chat_id: int) -> str:
    ...
    return (
        ...
        f"Multimedia bienvenida: <b>{'Sí' if media else 'No'}</b>\n"
        f"Botones bienvenida: <b>{len(buttons) if isinstance(buttons, list) else 0}</b>\n\n"
...
def admin_private_main_markup(chat_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"Validación {enabled}", callback_data=f"cfg|toggle_validation|{chat_id}")],
        [InlineKeyboardButton("👁 Vista previa bienvenida", callback_data=f"cfg|preview_public|{chat_id}")],
        [InlineKeyboardButton("✏️ Texto bienvenida", callback_data=f"cfg|edit_text|{chat_id}|validation_public_join_message")],
        [InlineKeyboardButton("🖼 Multimedia bienvenida", callback_data=f"cfg|media|{chat_id}"), InlineKeyboardButton("⌨️ Botones bienvenida", callback_data=f"cfg|buttons|{chat_id}")],
        [InlineKeyboardButton("❓ Preguntas", callback_data=f"cfg|questions|{chat_id}"), InlineKeyboardButton("👮 Quién valida", callback_data=f"cfg|approvers|{chat_id}")],
        [InlineKeyboardButton("📝 Mensaje preguntas", callback_data=f"cfg|edit_text|{chat_id}|validation_intro_message")],
        [InlineKeyboardButton("⏰ Recordatorio", callback_data=f"cfg|edit_text|{chat_id}|validation_reminder_message")],
        [InlineKeyboardButton("✅ Aprobado", callback_data=f"cfg|edit_text|{chat_id}|validation_approved_message"), InlineKeyboardButton("❌ Rechazado", callback_data=f"cfg|edit_text|{chat_id}|validation_rejected_message")],
...
async def admin_private_config_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if kind == "cfg_buttons":
        if text.upper() == "QUITAR":
            cfg_set(chat_id, "validation_public_join_buttons", [])
            pop_config_pending(update.effective_user.id)
        ...
        if not buttons:
            ...
            return True
        cfg_set(chat_id, "validation_public_join_buttons", buttons)
        pop_config_pending(update.effective_user.id)
    ...
    if kind == "cfg_media" and text.upper() == "QUITAR":
        cfg_set(chat_id, "validation_public_join_media", None)
        pop_config_pending(update.effective_user.id)
...
async def admin_private_config_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not media:
        ...
        return
    cfg_set(chat_id, "validation_public_join_media", media)
    pop_config_pending(update.effective_user.id)
    await update.message.reply_html("✅ Multimedia guardada para la bienvenida.", reply_markup=admin_private_main_markup(chat_id))
...
async def validation_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if action == "ok":
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text=str(cfg_value(chat_id, "validation_approved_message", VALIDATION_APPROVED_MESSAGE)).replace("{mention}", target_mention).replace("{name}", h(record.get("name", ""))),
            parse_mode=ParseMode.HTML,
        )
    ...
    if action == "no":
        ...
        try:
            ...
            return True
        try:
            await query.edit_message_text(str(cfg_value(chat_id, "validation_rejected_message", VALIDATION_REJECTED_MESSAGE)), parse_mode=ParseMode.HTML)
        except Exception:
            pass
...
async def validation_pending_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not pending:
        await update.message.reply_text("No hay presentaciones pendientes de validar.")
        return
