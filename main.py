        ...
        try:
            if isinstance(state_data, dict):
                if "auto_next_offset" not in state_data and "autoplay_offset" in state_data:
                    state_data["auto_next_offset"] = state_data.get("autoplay_offset", 0)
...
def auto_next_trigger_seconds(state: ChatState) -> int:
    if state.auto_next_offset < 0 and state.queue:
        return abs(int(state.auto_next_offset))
    return 0
...
def control_header(state: ChatState) -> str:
    ...
    auto_label = f"{state.auto_next_offset}s" if state.auto_next_offset >= 0 else str(state.auto_next_offset)
...
async def simulate_panel_next(chat_id: int, reason: str = "auto-next") -> bool:
    state = get_state(chat_id)
    if not state.now_playing:
        return False
    if state.paused:
    ...
    try:
        logger.info("Simulando pulsación de panel_next en chat %s (%s)", chat_id, reason)
        await VOICE.play_next_from_queue(chat_id)
        return True
    except Exception:
        logger.exception("Fallo al simular panel_next en chat %s", chat_id)
        return False


async def _recover_now_playing_duration(chat_id: int) -> int:
    state = get_state(chat_id)
    if not state.now_playing:
        return 0
    try:
    ...
    if track.local_path and Path(track.local_path).exists():
        duration = probe_duration_seconds(track.local_path)
        if duration > 0:
            track.duration = duration
            state.now_playing = asdict(track)
            save_all_states()
            logger.info("Duración recuperada para chat %s: %ss", chat_id, duration)
            return duration
    return 0
...
async def _auto_next_monitor(chat_id: int, track_token: str) -> None:
    ...
    try:
        while True:
            ...
            if VOICE.application and now_ts >= next_ui_refresh:
                next_ui_refresh = now_ts + UI_REFRESH_SECONDS
                try:
                    await ensure_panel(VOICE.application.bot, chat_id)
                    await ensure_control_panel(VOICE.application.bot, chat_id)
                except Exception:
            ...
            if remaining is not None:
                if remaining <= trigger:
                    await simulate_panel_next(chat_id, reason="timer_threshold")
                    return
                if remaining <= 0:
                    await simulate_panel_next(chat_id, reason="timer_end")
                    return
                continue

            # Si el tiempo sale como --:--, no abortamos: reintentamos recuperar duración
            if now_ts >= next_duration_recheck:
                next_duration_recheck = now_ts + UNKNOWN_DURATION_RECHECK_SECONDS
                duration = await _recover_now_playing_duration(chat_id)
                if duration > 0:
                    state = get_state(chat_id)
                    remaining = remaining_seconds(state)
                    if remaining is not None and remaining <= trigger:
                        await simulate_panel_next(chat_id, reason="recovered_duration_threshold")
                        return
                    if remaining is not None and remaining <= 0:
                        await simulate_panel_next(chat_id, reason="recovered_duration_end")
                        return
...
async def schedule_auto_next(chat_id: int, duration: int, offset: int) -> None:
    ...
    if not state.now_playing:
        return
    # Aunque duration sea 0, dejamos el monitor vivo para reintentar recuperar la duración
        ...
        while True:
            try:
                ...
                for chat_id, state in list(STATE_CACHE.items()):
                    ...
                    track = Track(**state.now_playing)
                    if int(track.duration or 0) <= 0 and track.local_path and Path(track.local_path).exists():
                        duration = probe_duration_seconds(track.local_path)
                        if duration > 0:
                            track.duration = duration
                            state.now_playing = asdict(track)
                    ...
                    duration = int(track.duration or 0)
                    if duration <= 0 or state.play_started_at is None:
                        continue

                    elapsed = max(0, int(_time.time() - state.play_started_at))
                    trigger = abs(state.auto_next_offset) if state.auto_next_offset < 0 else 0
                    deadline = max(0, duration - trigger)

                    if elapsed >= deadline:
                        logger.info(
                            "Watchdog auto-siguiente en chat %s | elapsed=%s duration=%s trigger=%s",
                            chat_id, elapsed, duration, trigger
                        )
                        await simulate_panel_next(chat_id, reason="watchdog_elapsed")
            except asyncio.CancelledError:
                return
            except Exception:
                logger.exception("Error en watchdog global de auto-siguiente")
    ...
    async def pause_resume(self, chat_id: int) -> None:
        ...
        if state.paused:
            ...
            if state.paused_remaining is not None and state.now_playing:
                track = Track(**state.now_playing)
                state.play_started_at = int(_time.time()) - max(0, int(track.duration) - int(state.paused_remaining))
                state.paused_remaining = None
            current = Track(**state.now_playing)
            save_all_states()
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "panel_next":
        ...
        if not state.queue:
            ...
            return
        await simulate_panel_next(chat_id, reason="manual_button")
        return
