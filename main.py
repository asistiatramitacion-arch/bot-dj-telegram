...
class ChatState:
    ...
    user_activity: Dict[str, Dict[str, Any]] = field(default_factory=dict)
...
DEFAULT_ADMIN_CONFIG: Dict[str, Any] = {
    ...
    "chat_title": "",
    "delete_commands_enabled": False,
    "delete_commands_after_seconds": 5,
    "pregonero_enabled": True,
}
...
def admin_panel_text(chat_id: int) -> str:
    ...
    return (
        ...
        f"Borrar comandos: <b>{bool_label(cfg.get('delete_commands_enabled'))}</b> ({cfg.get('delete_commands_after_seconds', 5)}s)\n\n"
...
def admin_panel_markup(chat_id: int) -> InlineKeyboardMarkup:
    cfg = admin_cfg(chat_id)
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🏓 Ping", callback_data="adm|ping"), InlineKeyboardButton("📚 Comandos", callback_data="adm|commands")],
        [InlineKeyboardButton(f"Validación {bool_label(cfg.get('validation_enabled'))}", callback_data="adm|toggle_validation")],
        [InlineKeyboardButton("📋 Pendientes", callback_data="adm|pendientes"), InlineKeyboardButton("🔇 Silenciados", callback_data="adm|silenced")],
        [InlineKeyboardButton("🆕 Últimas entradas", callback_data="adm|entries"), InlineKeyboardButton("📉 Inactivos 10d", callback_data="adm|inactive")],
        [InlineKeyboardButton("📊 Ranking", callback_data="adm|ranking"), InlineKeyboardButton("📢 Pregonero", callback_data="adm|pregonero")],
        [InlineKeyboardButton("📌 Normas", callback_data="adm|reglas")],
        ...
        [InlineKeyboardButton(f"Borrar comandos {bool_label(cfg.get('delete_commands_enabled'))}", callback_data="adm|toggle_delete_commands")],
        [InlineKeyboardButton("🕐 Borrar cmd -", callback_data="adm|cmd_del_minus"), InlineKeyboardButton("🕐 Borrar cmd +", callback_data="adm|cmd_del_plus")],
...
async def admin_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    elif action == "toggle_delete_commands":
        cfg_set(chat_id, "delete_commands_enabled", not bool(cfg.get("delete_commands_enabled")))
    elif action == "cmd_del_plus":
        cfg_set(chat_id, "delete_commands_after_seconds", min(120, int(cfg.get("delete_commands_after_seconds", 5)) + 1))
    elif action == "cmd_del_minus":
        cfg_set(chat_id, "delete_commands_after_seconds", max(0, int(cfg.get("delete_commands_after_seconds", 5)) - 1))
    elif action == "ping":
        await query.answer("✅ Bot activo", show_alert=True)
        return True
    elif action == "commands":
        await query.edit_message_text(all_commands_text(), parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data="adm|back")]]))
        return True
    elif action == "silenced":
        await query.edit_message_text(silenced_users_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data="adm|back")]]))
        return True
    elif action == "entries":
        await query.edit_message_text(latest_entries_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data="adm|back")]]))
        return True
    elif action == "inactive":
        await query.edit_message_text(inactive_users_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data="adm|back")]]))
        return True
    elif action == "ranking":
        await query.edit_message_text(ranking_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data="adm|back")]]))
        return True
    elif action == "pregonero":
        await pregonero_send_known_members(context, chat_id, "📢 <b>EL PLAN TE LLAMA</b>")
        await query.answer("Pregonero enviado a usuarios conocidos.", show_alert=True)
        return True
    elif action == "reglas":
...
def all_commands_text() -> str:
    return (
        ...
        "<code>/ajustes</code> - abrir panel privado de configuración\n\n"
        "<b>Panel del grupo</b>\n"
        "<code>/plan</code> - panel de administración\n"
        "<code>/ping</code> - comprobar que el bot responde\n\n"
        "<b>Validación</b>\n"
        ...
        "<code>/preséntate</code> - también funciona escrito como texto\n"
        "<code>/setpreguntas Nombre:|Edad:|Lugar:|¿Qué buscas?</code>\n"
        ...
        "<b>Listados</b>\n"
        "<code>/silenciados</code> - usuarios silenciados/pendientes\n"
        "<code>/ultimasentradas</code> - últimas entradas registradas\n"
        "<code>/inactivos</code> - sin actividad en 10 días\n"
        "<code>/ranking</code> - ranking de actividad\n\n"
        "<b>Pregonero</b>\n"
        "<code>/el plan te llama</code> - menciona usuarios conocidos por el bot\n\n"
        "<b>Moderación</b>\n"
...
def admin_private_main_markup(chat_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🏓 Ping", callback_data=f"cfg|ping|{chat_id}"), InlineKeyboardButton("📚 Ver comandos", callback_data=f"cfg|commands|{chat_id}")],
        [InlineKeyboardButton(f"Validación {enabled}", callback_data=f"cfg|toggle_validation|{chat_id}"), InlineKeyboardButton("👮 Quién valida", callback_data=f"cfg|approvers|{chat_id}")],
        [InlineKeyboardButton("👥 Pendientes", callback_data=f"cfg|pending|{chat_id}"), InlineKeyboardButton("🔇 Silenciados", callback_data=f"cfg|silenced|{chat_id}")],
        [InlineKeyboardButton("🆕 Entradas", callback_data=f"cfg|entries|{chat_id}"), InlineKeyboardButton("📉 Inactivos 10d", callback_data=f"cfg|inactive|{chat_id}")],
        [InlineKeyboardButton("📊 Ranking", callback_data=f"cfg|ranking|{chat_id}"), InlineKeyboardButton("📢 Pregonero", callback_data=f"cfg|pregonero|{chat_id}")],
        ...
        [InlineKeyboardButton(f"Borrar comandos {bool_label(cfg_value(chat_id, 'delete_commands_enabled'))}", callback_data=f"cfg|toggle_delcmd|{chat_id}")],
        [InlineKeyboardButton("🕐 Cmd -", callback_data=f"cfg|delcmd_minus|{chat_id}"), InlineKeyboardButton("🕐 Cmd +", callback_data=f"cfg|delcmd_plus|{chat_id}")],
...
async def admin_private_config_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif action == "toggle_delcmd":
        cfg_set(chat_id, "delete_commands_enabled", not bool(cfg_value(chat_id, "delete_commands_enabled")))
    elif action == "delcmd_plus":
        cfg_set(chat_id, "delete_commands_after_seconds", min(120, int(cfg_value(chat_id, "delete_commands_after_seconds", 5)) + 1))
    elif action == "delcmd_minus":
        cfg_set(chat_id, "delete_commands_after_seconds", max(0, int(cfg_value(chat_id, "delete_commands_after_seconds", 5)) - 1))
    elif action == "ping":
        await query.answer("✅ Bot activo", show_alert=True)
        return
    elif action == "silenced":
        await query.edit_message_text(silenced_users_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data=f"cfg|open|{chat_id}")]]))
        return
    elif action == "entries":
        await query.edit_message_text(latest_entries_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data=f"cfg|open|{chat_id}")]]))
        return
    elif action == "inactive":
        await query.edit_message_text(inactive_users_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data=f"cfg|open|{chat_id}")]]))
        return
    elif action == "ranking":
        await query.edit_message_text(ranking_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data=f"cfg|open|{chat_id}")]]))
        return
    elif action == "pregonero":
        await pregonero_send_known_members(context, chat_id, "📢 <b>EL PLAN TE LLAMA</b>")
        await query.answer("Pregonero enviado a usuarios conocidos.", show_alert=True)
...
async def validation_start_for_user(context: ContextTypes.DEFAULT_TYPE, chat_id: int, chat_title: str, user, *, reply_to_message_id: Optional[int] = None, source: str = "new_member") -> None:
    if not validation_is_active_for_chat(chat_id) or getattr(user, "is_bot", False):
        return
    ...
    remember_chat_title(chat_id, chat_title or "")
    try:
        public_message_id = await send_configured_public_join(context.bot, chat_id, user, reply_to_message_id=reply_to_message_id)
...
async def validation_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for user in update.message.new_chat_members:
        await validation_start_for_user(context, chat_id, update.effective_chat.title or "", user, reply_to_message_id=update.message.message_id, source="new_member")


async def validation_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not validation_is_active_for_chat(chat_id):
        return
    remember_chat_title(chat_id, getattr(chat, "title", "") or "")
    try:
        await context.bot.approve_chat_join_request(chat_id=chat_id, user_id=user.id)
    except Exception:
        logger.exception("No se pudo aprobar solicitud de entrada de %s en %s", user.id, chat_id)
        return
    await asyncio.sleep(1)
    await validation_start_for_user(context, chat_id, getattr(chat, "title", "") or "", user, reply_to_message_id=None, source="join_request")
...
async def validation_chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    old_status = getattr(cmu.old_chat_member, "status", "")
    new_status = getattr(cmu.new_chat_member, "status", "")
    if new_status in ("member", "restricted") and old_status in ("left", "kicked"):
        await validation_start_for_user(context, chat.id, getattr(chat, "title", "") or "", user, reply_to_message_id=None, source="chat_member_update")
...
async def force_presentate_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not update.message.reply_to_message or not update.message.reply_to_message.from_user:
        await update.message.reply_text("Responde al usuario con /presentate o /preséntate.")
        return
    target = update.message.reply_to_message.from_user
    await validation_start_for_user(context, chat_id, update.effective_chat.title or "", target, reply_to_message_id=update.message.reply_to_message.message_id, source="forced")
    await update.message.reply_html(f"🔇 {target.mention_html()} queda obligado a presentarse.")
...
def activity_touch(chat_id: int, user) -> None:
    if not user or getattr(user, "is_bot", False):
        return
    state = get_state(chat_id)
    if not isinstance(getattr(state, "user_activity", None), dict):
        state.user_activity = {}
    uid = str(user.id)
    now = _now_ts()
    rec = state.user_activity.get(uid, {})
    rec.setdefault("first_seen", now)
    rec["last_seen"] = now
    rec["name"] = display_name(user)
    rec["username"] = f"@{user.username}" if getattr(user, "username", None) else ""
    rec["message_count"] = int(rec.get("message_count", 0)) + 1
    state.user_activity[uid] = rec
    save_all_states()
...
async def activity_tracker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat or not update.effective_user:
        return
    if update.effective_chat.type == "private" or not chat_is_allowed(update.effective_chat.id):
        return
    activity_touch(update.effective_chat.id, update.effective_user)


async def command_cleanup_tracker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat or not update.message or update.effective_chat.type == "private":
        return
    chat_id = update.effective_chat.id
    if not chat_is_allowed(chat_id):
        return
    if not bool(cfg_value(chat_id, "delete_commands_enabled", False)):
        return
    delay = max(0, int(cfg_value(chat_id, "delete_commands_after_seconds", 5)))
    async def _delete():
        if delay:
...
def silenced_users_text(chat_id: int) -> str:
    state = get_state(chat_id)
    rows = [(uid, r) for uid, r in state.validation_users.items() if r.get("status") in ("answering", "pending_admin")]
    if not rows:
        return "<b>🔇 Silenciados</b>\n\nNo hay usuarios silenciados por validación."
    lines = ["<b>🔇 Silenciados por validación</b>", ""]
    for uid, r in sorted(rows, key=lambda x: int(x[1].get("joined_ts", 0)), reverse=True)[:50]:
        status = "contestando" if r.get("status") == "answering" else "pendiente admin"
        lines.append(f"• <a href=\"tg://user?id={uid}\">{h(r.get('name') or uid)}</a> — {status} — {_fmt_ts(r.get('joined_ts'))}")
    return "\n".join(lines)


def latest_entries_text(chat_id: int) -> str:
    state = get_state(chat_id)
    rows = list(state.validation_users.items())
    if not rows:
        return "<b>🆕 Últimas entradas</b>\n\nTodavía no hay entradas registradas."
    lines = ["<b>🆕 Últimas entradas registradas</b>", ""]
    for uid, r in sorted(rows, key=lambda x: int(x[1].get("joined_ts", 0)), reverse=True)[:30]:
        lines.append(f"• <a href=\"tg://user?id={uid}\">{h(r.get('name') or uid)}</a> — {h(r.get('status',''))} — {_fmt_ts(r.get('joined_ts'))}")
    return "\n".join(lines)


def inactive_users_text(chat_id: int, days: int = 10) -> str:
    state = get_state(chat_id)
    cutoff = _now_ts() - days * 86400
    rows = [(uid, r) for uid, r in getattr(state, "user_activity", {}).items() if int(r.get("last_seen", 0)) < cutoff]
    if not rows:
        return f"<b>📉 Inactivos {days} días</b>\n\nNo tengo usuarios inactivos registrados. Solo puedo medir actividad desde que el bot está activo."
    lines = [f"<b>📉 Inactivos últimos {days} días</b>", ""]
    for uid, r in sorted(rows, key=lambda x: int(x[1].get("last_seen", 0)))[:50]:
        lines.append(f"• <a href=\"tg://user?id={uid}\">{h(r.get('name') or uid)}</a> — última actividad: {_fmt_ts(r.get('last_seen'))}")
    return "\n".join(lines)


def ranking_text(chat_id: int) -> str:
    state = get_state(chat_id)
    rows = list(getattr(state, "user_activity", {}).items())
    if not rows:
        return "<b>📊 Ranking</b>\n\nAún no tengo actividad registrada."
    lines = ["<b>📊 Ranking de actividad</b>", ""]
    for i, (uid, r) in enumerate(sorted(rows, key=lambda x: int(x[1].get("message_count", 0)), reverse=True)[:30], start=1):
        lines.append(f"{i}. <a href=\"tg://user?id={uid}\">{h(r.get('name') or uid)}</a> — {int(r.get('message_count', 0))} mensajes")
    return "\n".join(lines)


def known_member_mentions(chat_id: int) -> List[str]:
    state = get_state(chat_id)
    ids = set(getattr(state, "user_activity", {}).keys()) | set(state.validation_users.keys())
    result = []
    for uid in sorted(ids):
        name = None
        if uid in getattr(state, "user_activity", {}):
            name = state.user_activity[uid].get("name")
        if not name and uid in state.validation_users:
            name = state.validation_users[uid].get("name")
        result.append(f"<a href=\"tg://user?id={uid}\">{h(name or uid)}</a>")
    return result


async def pregonero_send_known_members(context: ContextTypes.DEFAULT_TYPE, chat_id: int, header: str = "📢 <b>EL PLAN TE LLAMA</b>") -> None:
    mentions = known_member_mentions(chat_id)
    if not mentions:
        await context.bot.send_message(chat_id=chat_id, text="No tengo usuarios registrados todavía.")
        return
    chunk = header + "\n\n"
    for mention in mentions:
        add = mention + " "
        if len(chunk) + len(add) > 3500:
...
async def ping_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.message:
        await update.message.reply_text("🏓 Pong. Bot activo correctamente.")


async def silenced_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat or not update.effective_user or not update.message:
        return
    if not await is_admin(context, update.effective_chat.id, update.effective_user.id):
        await update.message.reply_text("Solo administradores.")
        return
    await update.message.reply_html(silenced_users_text(update.effective_chat.id))


async def latest_entries_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat or not update.effective_user or not update.message:
        return
    if not await is_admin(context, update.effective_chat.id, update.effective_user.id):
        await update.message.reply_text("Solo administradores.")
        return
    await update.message.reply_html(latest_entries_text(update.effective_chat.id))


async def inactive_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat or not update.effective_user or not update.message:
        return
    if not await is_admin(context, update.effective_chat.id, update.effective_user.id):
        await update.message.reply_text("Solo administradores.")
        return
    await update.message.reply_html(inactive_users_text(update.effective_chat.id))
...
async def ranking_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await is_admin(context, update.effective_chat.id, update.effective_user.id):
        await update.message.reply_text("Solo administradores.")
        return
    await update.message.reply_html(ranking_text(update.effective_chat.id))
...
async def pregonero_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    args = " ".join(context.args).lower().strip() if hasattr(context, "args") else ""
    if text.startswith("/el plan te llama") or args.startswith("plan te llama"):
        await pregonero_send_known_members(context, update.effective_chat.id)
    else:
        await update.message.reply_text("Usa: /el plan te llama")
...
def build_application() -> Application:
    ...
    application.add_handler(MessageHandler(filters.COMMAND, command_cleanup_tracker), group=-100)
    application.add_handler(MessageHandler(filters.ALL, activity_tracker), group=-90)

    application.add_handler(CommandHandler(["Djplan", "djplan", "start"], start_command))
    application.add_handler(CommandHandler("dj", assign_dj_command))
    application.add_handler(CommandHandler(["plan", "admin", "paneladmin"], admin_command))
    application.add_handler(CommandHandler(["ajustes", "configuracion", "config"], admin_private_config_command))
    application.add_handler(CommandHandler("ping", ping_command))
    application.add_handler(CommandHandler("presentate", force_presentate_command))
    application.add_handler(MessageHandler(filters.Regex(r"^/pres[ée]ntate(?:@\w+)?(?:\s|$)"), force_presentate_command))
    application.add_handler(CommandHandler("el", pregonero_command))
    application.add_handler(CommandHandler("silenciados", silenced_command))
    application.add_handler(CommandHandler("ultimasentradas", latest_entries_command))
    application.add_handler(CommandHandler("inactivos", inactive_command))
    ...
    application.add_handler(ChatJoinRequestHandler(validation_join_request), group=-20)
    application.add_handler(ChatMemberHandler(validation_chat_member_update, ChatMemberHandler.CHAT_MEMBER), group=-20)
    application.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, validation_new_member), group=-20)
