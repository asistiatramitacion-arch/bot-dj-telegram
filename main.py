...
def cfg_back_markup(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver al panel", callback_data=f"cfg|open|{chat_id}")]])
...
def admin_private_main_text(chat_id: int) -> str:
    ...
    return (
        ...
        f"Silenciados: <b>{len(state.muted_users)}</b> · Expulsados: <b>{len(state.expelled_users)}</b>\n"
        f"Pregonero manual: <b>{manual_count}</b> · Borrado comandos: <b>{h(command_cleanup_label(chat_id))}</b>\n\n"
        "Los bloques anchos son separadores. Las vistas previas envían texto + multimedia + botones reales al privado."
    )


def admin_private_main_markup(chat_id: int) -> InlineKeyboardMarkup:
    enabled = cfg_status(cfg_value(chat_id, "validation_enabled"))
    autoapprove = cfg_status(cfg_value(chat_id, "validation_auto_approve_join_requests"))
    kick = cfg_status(cfg_value(chat_id, "validation_kick_if_timeout"))
    farewell = cfg_status(cfg_value(chat_id, "farewell_enabled"))
    rules_auto = cfg_status(cfg_value(chat_id, "rules_auto_after_approve"))
    rows: List[List[InlineKeyboardButton]] = []

    rows.append(section_button("🛡️ BLOQUE VALIDACIÓN", chat_id))
    rows.append([
        InlineKeyboardButton(f"{enabled} Validación", callback_data=f"cfg|toggle_validation|{chat_id}"),
        InlineKeyboardButton(f"{autoapprove} Autoaprobar", callback_data=f"cfg|toggle_autoapprove|{chat_id}"),
        InlineKeyboardButton(f"{kick} Expulsar timeout", callback_data=f"cfg|toggle_kick|{chat_id}"),
    ])
    rows.append([
        InlineKeyboardButton(f"⏱ {cfg_value(chat_id, 'validation_timeout_minutes')}m -", callback_data=f"cfg|time_minus|{chat_id}"),
        InlineKeyboardButton(f"⏱ Tiempo +", callback_data=f"cfg|time_plus|{chat_id}"),
        InlineKeyboardButton("👮 Quién valida", callback_data=f"cfg|approvers|{chat_id}"),
    ])
    rows.append([
        InlineKeyboardButton(f"🔔 {cfg_value(chat_id, 'validation_reminder_minutes')}m -", callback_data=f"cfg|rem_minus|{chat_id}"),
        InlineKeyboardButton("🔔 Record +", callback_data=f"cfg|rem_plus|{chat_id}"),
        InlineKeyboardButton(f"🧹 Cmd {command_cleanup_label(chat_id)}", callback_data=f"cfg|cleanup_mode|{chat_id}"),
    ])

    rows.append(section_button("👋 BLOQUE BIENVENIDA", chat_id))
    rows.append(preview_button(chat_id, "welcome", "👁 VISTA PREVIA COMPLETA BIENVENIDA"))
    rows.extend(profile_control_rows(chat_id, "welcome", "✏️ Texto"))

    rows.append(section_button("❓ BLOQUE PREGUNTAS", chat_id))
    rows.append(preview_button(chat_id, "questions", "👁 VISTA PREVIA COMPLETA PREGUNTAS"))
    rows.extend(profile_control_rows(chat_id, "questions", "📝 Texto"))
    rows.append([InlineKeyboardButton("❓ Editar preguntas", callback_data=f"cfg|questions|{chat_id}")])

    rows.append(section_button("🔔 BLOQUE RECORDATORIO", chat_id))
    rows.append(preview_button(chat_id, "reminder", "👁 VISTA PREVIA COMPLETA RECORDATORIO"))
    rows.extend(profile_control_rows(chat_id, "reminder", "⏰ Texto"))

    rows.append(section_button("⛔ BLOQUE TIMEOUT", chat_id))
    rows.append(preview_button(chat_id, "timeout", "👁 VISTA PREVIA COMPLETA TIMEOUT"))
    rows.extend(profile_control_rows(chat_id, "timeout", "⏱ Texto"))

    rows.append(section_button("✅ BLOQUE APROBADO", chat_id))
    rows.append(preview_button(chat_id, "approved", "👁 VISTA PREVIA COMPLETA APROBADO"))
    rows.extend(profile_control_rows(chat_id, "approved", "✅ Texto"))
    rows.append([InlineKeyboardButton(f"{rules_auto} Mostrar normas al aprobar", callback_data=f"cfg|toggle_rules_after_approve|{chat_id}")])

    rows.append(section_button("❌ BLOQUE RECHAZADO / EXPULSADO", chat_id))
    rows.append(preview_button(chat_id, "rejected", "👁 VISTA PREVIA COMPLETA RECHAZADO"))
    rows.extend(profile_control_rows(chat_id, "rejected", "❌ Texto"))

    rows.append(section_button("📌 BLOQUE NORMAS", chat_id))
    rows.append(preview_button(chat_id, "rules", "👁 VISTA PREVIA COMPLETA NORMAS"))
    rows.extend(profile_control_rows(chat_id, "rules", "📌 Texto"))

    rows.append(section_button("📣 BLOQUE PREGONERO", chat_id))
    rows.append(preview_button(chat_id, "pregonero", "👁 VISTA PREVIA COMPLETA PREGONERO"))
    rows.extend(profile_control_rows(chat_id, "pregonero", "📣 Texto"))
    rows.append([
        InlineKeyboardButton("➕ Users manual", callback_data=f"cfg|pregonero_manual|{chat_id}"),
        InlineKeyboardButton("🧹 Quitar manual", callback_data=f"cfg|pregonero_clear_manual|{chat_id}"),
        InlineKeyboardButton("🚀 Lanzar", callback_data=f"cfg|send_pregonero|{chat_id}"),
    ])

    rows.append(section_button("👋 BLOQUE DESPEDIDA", chat_id))
    rows.append([InlineKeyboardButton(f"{farewell} Despedida automática", callback_data=f"cfg|toggle_farewell|{chat_id}")])
    rows.append(preview_button(chat_id, "farewell", "👁 VISTA PREVIA COMPLETA DESPEDIDA"))
    rows.extend(profile_control_rows(chat_id, "farewell", "👋 Texto"))

    rows.append(section_button("📊 LISTADOS Y CONTROL", chat_id))
    rows.append([
    ...
    rows.append([InlineKeyboardButton("🏓 PING ANCHO", callback_data=f"cfg|ping|{chat_id}")])
    rows.append([
...
async def admin_private_config_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "toggle_validation":
        ...
        await query.edit_message_text(all_commands_text(), parse_mode=ParseMode.HTML, reply_markup=cfg_back_markup(chat_id))
        return
    elif action == "cleanup_mode":
        cfg_set(chat_id, "command_cleanup_mode", next_command_cleanup_mode(chat_id))
    elif action == "muted":
        await query.edit_message_text(admin_muted_users_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=cfg_back_markup(chat_id))
        return
    elif action == "entries":
        await query.edit_message_text(admin_last_entries_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=cfg_back_markup(chat_id))
        return
    elif action == "inactive":
        await query.edit_message_text(admin_inactive_users_text(chat_id, 10), parse_mode=ParseMode.HTML, reply_markup=cfg_back_markup(chat_id))
        return
    elif action == "ranking":
        await query.edit_message_text(admin_ranking_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=cfg_back_markup(chat_id))
        return
    elif action == "expelled":
        await query.edit_message_text(admin_expelled_users_text(chat_id), parse_mode=ParseMode.HTML, reply_markup=cfg_back_markup(chat_id))
        return
    elif action == "log":
        await query.edit_message_text(admin_action_log_text(chat_id, 3), parse_mode=ParseMode.HTML, reply_markup=cfg_back_markup(chat_id))
        return
    elif action == "pending":
        ...
        set_config_pending(update.effective_user.id, {"kind": "cfg_pregonero_manual", "chat_id": chat_id})
        await query.edit_message_text(
...
async def admin_private_config_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if kind == "cfg_pregonero_manual":
        ...
        await update.message.reply_html(f"✅ Usuarios manuales del pregonero guardados: <b>{len(users[:300])}</b>", reply_markup=admin_private_main_markup(chat_id))
        return True
    return False
