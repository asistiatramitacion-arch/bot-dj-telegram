...
# =========================================================
# V39 - MANTENIMIENTO Y SELECTOR PRIVADO ROBUSTO
# No borra configuraciones de grupos. Solo repara/limpia vínculos privados.
# =========================================================
from telegram.ext import ApplicationHandlerStop


def _v39_is_group_chat_id(value: Any) -> bool:
    try:
        cid = int(value)
        # Los grupos/supergrupos/canales de Telegram tienen ID negativo para el bot.
        return cid < 0
    except Exception:
        return False


def _v39_title_for_chat(chat_id: int) -> str:
    try:
        title = str(cfg_value(int(chat_id), "chat_title", "") or "").strip()
        return title or str(chat_id)
    except Exception:
        return str(chat_id)


def _v39_private_candidate_chat_ids(user_id: Optional[int] = None) -> List[int]:
    ids: set[int] = set()
    try:
        for cid in list(STATE_CACHE.keys()):
            if _v39_is_group_chat_id(cid):
                ids.add(int(cid))
    except Exception:
        pass
    ...
    if user_id is not None:
        try:
            hot_load_private_groups()
            for cid, _title in hot_private_groups_for(int(user_id)):
                if _v39_is_group_chat_id(cid):
                    ids.add(int(cid))
        except Exception:
            pass
        try:
            dj_load_private_groups()
            for cid in dj_private_groups_for(int(user_id)).keys():
                if _v39_is_group_chat_id(cid):
                    ids.add(int(cid))
        except Exception:
            pass
...
def _v39_private_selector_markup(user_id: int) -> InlineKeyboardMarkup:
    rows: List[List[InlineKeyboardButton]] = []
    for chat_id in _v39_private_candidate_chat_ids(user_id):
        label = _v39_title_for_chat(chat_id)
        if len(label) > 45:
            label = label[:42] + "…"
        rows.append([InlineKeyboardButton(f"⚙️ {label}", callback_data=f"v39cfg|open|{chat_id}")])
    rows.append([InlineKeyboardButton("🧰 Reparar vínculos", callback_data="v39cfg|fix")])
    rows.append([InlineKeyboardButton("🧹 Limpiar vínculos privados", callback_data="v39cfg|clear")])
    return InlineKeyboardMarkup(rows)


async def _v39_refresh_chat_titles(context: ContextTypes.DEFAULT_TYPE, chat_ids: List[int]) -> Tuple[int, List[str]]:
    ok = 0
    errors: List[str] = []
    for cid in chat_ids:
        try:
            ...
            cfg_set(int(cid), "chat_title", str(title))
            ok += 1
        except Exception as e:
...
def _v39_clean_private_links_for_user(user_id: int, *, clear_all: bool = False) -> Dict[str, Any]:
    """Limpia solo vínculos privados HOT/DJ. No toca state.json de configuración de grupos."""
    result = {"hot_before": 0, "hot_after": 0, "dj_before": 0, "dj_after": 0}
    uid = str(int(user_id))

    # HOT
    try:
        hot_load_private_groups()
        rec = HOT_PRIVATE_GROUPS.get(uid)
        if isinstance(rec, dict):
            groups = rec.get("groups", {}) if isinstance(rec.get("groups"), dict) else {}
            result["hot_before"] = len(groups)
            if clear_all:
                rec["groups"] = {}
                rec["selected"] = None
            else:
                fixed = {str(int(k)): str(v) for k, v in groups.items() if _v39_is_group_chat_id(k)}
                rec["groups"] = fixed
                selected = rec.get("selected")
                try:
                    rec["selected"] = int(selected) if selected is not None and str(int(selected)) in fixed else (int(next(iter(fixed))) if fixed else None)
                except Exception:
                    rec["selected"] = int(next(iter(fixed))) if fixed else None
            result["hot_after"] = len(rec.get("groups", {}) or {})
            hot_save_private_groups()
    except Exception:
        logger.exception("V39: no se pudo limpiar HOT_PRIVATE_GROUPS")

    # DJ
    try:
        dj_load_private_groups()
        rec = DJ_PRIVATE_GROUPS.get(uid)
        if isinstance(rec, dict):
            groups = rec.get("groups", {}) if isinstance(rec.get("groups"), dict) else {}
            result["dj_before"] = len(groups)
            if clear_all:
                rec["groups"] = {}
                rec["selected"] = None
            else:
                ...
                try:
                    ...
                    rec["selected"] = int(next(iter(fixed))) if fixed else None
            result["dj_after"] = len(rec.get("groups", {}) or {})
            dj_save_private_groups()
    except Exception:
        logger.exception("V39: no se pudo limpiar DJ_PRIVATE_GROUPS")

    return result


async def mantenimiento_fixprivado_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo ADMIN_IDS puede usar mantenimiento privado.")
        raise ApplicationHandlerStop
    load_all_states()
    stats = _v39_clean_private_links_for_user(update.effective_user.id, clear_all=False)
    chat_ids = _v39_private_candidate_chat_ids(update.effective_user.id)
    ok, errors = await _v39_refresh_chat_titles(context, chat_ids)
    lines = [
        "<b>🧰 Reparación privada completada</b>",
        "",
        f"Grupos detectados: <b>{len(chat_ids)}</b>",
        f"Títulos refrescados: <b>{ok}</b>",
        f"HOT vínculos: <b>{stats['hot_before']} → {stats['hot_after']}</b>",
        f"DJ vínculos: <b>{stats['dj_before']} → {stats['dj_after']}</b>",
    ]
    if errors:
        lines.append("")
        lines.append("<i>Avisos:</i>")
        for err in errors[:8]:
            lines.append(f"• {h(err)}")
    lines.append("")
    lines.append("Pulsa un grupo para abrir ajustes:")
    await update.message.reply_html("\n".join(lines), reply_markup=_v39_private_selector_markup(update.effective_user.id))
    raise ApplicationHandlerStop


async def mantenimiento_limpiarprivado_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo ADMIN_IDS puede usar mantenimiento privado.")
        raise ApplicationHandlerStop
    stats = _v39_clean_private_links_for_user(update.effective_user.id, clear_all=True)
    await update.message.reply_html(
        "<b>🧹 Vínculos privados limpiados</b>\n\n"
        "No he borrado configuraciones del grupo, recurrentes, DJ, listas ni validación.\n\n"
        f"HOT vínculos: <b>{stats['hot_before']} → 0</b>\n"
        f"DJ vínculos: <b>{stats['dj_before']} → 0</b>\n\n"
        "Ahora entra en cada grupo y ejecuta:\n"
        "<code>/hotgrupo</code>\n<code>/djgrupo</code>\n\n"
        "Para /ajustes basta con que el grupo exista en /data/state.json.",
    )
    raise ApplicationHandlerStop


async def mantenimiento_debugajustes_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo ADMIN_IDS puede usar debug.")
        raise ApplicationHandlerStop
    load_all_states()
    hot_load_private_groups()
    dj_load_private_groups()
    uid = str(int(update.effective_user.id))
    state_ids = sorted([int(x) for x in STATE_CACHE.keys() if _v39_is_group_chat_id(x)])
    hot_ids = [cid for cid, _t in hot_private_groups_for(update.effective_user.id)]
    dj_ids = list(dj_private_groups_for(update.effective_user.id).keys())
    lines = [
        "<b>🔎 Debug ajustes privados</b>",
        "",
        f"STATE grupos: <b>{len(state_ids)}</b>",
        f"HOT grupos vinculados: <b>{len(hot_ids)}</b>",
        f"DJ grupos vinculados: <b>{len(dj_ids)}</b>",
        "",
        "<b>Grupos disponibles:</b>",
    ]
    for cid in _v39_private_candidate_chat_ids(update.effective_user.id)[:30]:
        lines.append(f"• <code>{cid}</code> — {h(_v39_title_for_chat(cid))}")
    if len(_v39_private_candidate_chat_ids(update.effective_user.id)) > 30:
        lines.append("…")
    await update.message.reply_html("\n".join(lines), reply_markup=_v39_private_selector_markup(update.effective_user.id))
    raise ApplicationHandlerStop


async def mantenimiento_ajustes_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Intercepta /ajustes solo en privado para evitar el selector antiguo corrupto.
    if not update.effective_user or not update.message:
        return
    if update.effective_chat and update.effective_chat.type != "private":
        return  # deja actuar al handler original en grupos
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo ADMIN_IDS puede abrir la configuración privada.")
        raise ApplicationHandlerStop
    load_all_states()
    ...
    if not chat_ids:
        await update.message.reply_html(
            "No encuentro grupos válidos en el estado.\n\n"
            "Entra en el grupo y ejecuta <code>/plan</code>. Para HOT/DJ usa también <code>/hotgrupo</code> y <code>/djgrupo</code>."
        )
        raise ApplicationHandlerStop
    await update.message.reply_html(
        "<b>Elige el grupo que quieres configurar:</b>\n\n"
        "Si un botón no abre, usa <code>/fixprivado</code> y vuelve a probar.",
        reply_markup=_v39_private_selector_markup(update.effective_user.id),
    )
    raise ApplicationHandlerStop


async def mantenimiento_private_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query or not update.effective_user:
        return
    data = query.data or ""
    ...
    if data == "v39cfg|fix":
        ...
        await query.message.reply_html(
            "✅ Reparado.\n"
            f"HOT: <b>{stats['hot_before']} → {stats['hot_after']}</b> · "
            f"DJ: <b>{stats['dj_before']} → {stats['dj_after']}</b>",
            reply_markup=_v39_private_selector_markup(update.effective_user.id),
        )
        raise ApplicationHandlerStop

    if data == "v39cfg|clear":
        stats = _v39_clean_private_links_for_user(update.effective_user.id, clear_all=True)
        await query.message.reply_html(
            "🧹 Vínculos HOT/DJ limpiados. No se han borrado configuraciones del grupo.\n"
            "Vuelve a enlazar con /hotgrupo y /djgrupo desde cada grupo.",
            reply_markup=_v39_private_selector_markup(update.effective_user.id),
        )
        raise ApplicationHandlerStop

    if data.startswith("v39cfg|open|"):
        try:
            chat_id = int(data.split("|", 2)[2])
        except Exception:
            await query.message.reply_text("ID de grupo inválido.")
            raise ApplicationHandlerStop
        if not _v39_is_group_chat_id(chat_id):
            await query.message.reply_text("Ese ID no parece un grupo válido.")
            raise ApplicationHandlerStop
        try:
            # Actualiza selected HOT/DJ si ese grupo está vinculado, pero no obliga a que lo esté.
            try:
                hot_select_private_group(update.effective_user.id, chat_id)
            except Exception:
                pass
            try:
            ...
            # Enviar panel nuevo es más robusto que editar un mensaje antiguo con callbacks viejos.
            await query.message.reply_html(text, reply_markup=markup)
        except Exception as e:
            logger.exception("V39: fallo abriendo ajustes del chat %s", chat_id)
            await query.message.reply_html(
                "❌ No pude abrir ese panel.\n\n"
                f"Grupo: <code>{chat_id}</code>\n"
                f"Error: <code>{h(type(e).__name__)}: {h(str(e))}</code>\n\n"
                "Prueba <code>/fixprivado</code>. Si sigue, manda captura del log de Railway.",
                reply_markup=_v39_private_selector_markup(update.effective_user.id),
            )
        raise ApplicationHandlerStop


_old_build_application_v39 = build_application

def build_application() -> Application:
    app = _old_build_application_v39()
    # Grupo muy prioritario: estos handlers se ejecutan antes que los antiguos.
    app.add_handler(CommandHandler(["ajustes", "configuracion", "config", "admin"], mantenimiento_ajustes_command), group=-100)
    app.add_handler(CommandHandler(["fixprivado", "repararprivado"], mantenimiento_fixprivado_command), group=-100)
    app.add_handler(CommandHandler("limpiarprivado", mantenimiento_limpiarprivado_command), group=-100)
    app.add_handler(CommandHandler("debugajustes", mantenimiento_debugajustes_command), group=-100)
    app.add_handler(CallbackQueryHandler(mantenimiento_private_callback, pattern=r"^v39cfg\|"), group=-100)
    return app
