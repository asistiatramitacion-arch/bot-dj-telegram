...
def validation_is_active_for_chat(chat_id: int) -> bool:
    return VALIDATION_ENABLED and chat_is_allowed(chat_id)
...
async def validation_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for user in update.message.new_chat_members:
        ...
        record = {
            ...
            "answers": [],
            "joined_ts": joined,
            "deadline_ts": joined + VALIDATION_TIMEOUT_MINUTES * 60,
            "reminder_ts": joined + VALIDATION_REMINDER_MINUTES * 60,
            "reminded": False,
        ...
        try:
            public_msg = await context.bot.send_message(
                chat_id=chat_id,
                text=validation_format_template(VALIDATION_PUBLIC_JOIN_MESSAGE, user),
                parse_mode=ParseMode.HTML,
            )
            record["public_message_id"] = public_msg.message_id
        except Exception:
        ...
        try:
            q_msg = await context.bot.send_message(
                chat_id=chat_id,
                text=(
                    validation_format_template(VALIDATION_INTRO_MESSAGE, user)
                    + f"\n\n<b>Pregunta 1/{len(VALIDATION_QUESTIONS)}</b>\n{h(VALIDATION_QUESTIONS[0])}"
                ),
                parse_mode=ParseMode.HTML,
...
async def validation_handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if status == "pending_admin":
        if VALIDATION_DELETE_WRONG_MESSAGES:
    ...
    current_question = VALIDATION_QUESTIONS[step] if 0 <= step < len(VALIDATION_QUESTIONS) else "Respuesta"
    answers.append({"question": current_question, "answer": text})
    step += 1
    record["answers"] = answers
    record["step"] = step
    if step < len(VALIDATION_QUESTIONS):
        record["status"] = "answering"
        validation_set_record(chat_id, user.id, record)
        await update.message.reply_html(
            f"✅ Recibido.\n\n<b>Pregunta {step + 1}/{len(VALIDATION_QUESTIONS)}</b>\n{h(VALIDATION_QUESTIONS[step])}"
        )
        return True
...
async def validation_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if action == "ok":
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text=VALIDATION_APPROVED_MESSAGE.replace("{mention}", target_mention).replace("{name}", h(record.get("name", ""))),
            parse_mode=ParseMode.HTML,
        )
    ...
    if action == "no":
        ...
        try:
            ...
            return True
        try:
            await query.edit_message_text(VALIDATION_REJECTED_MESSAGE, parse_mode=ParseMode.HTML)
        except Exception:
            pass
                ...
                for user_id_str, record in list(state.validation_users.items()):
                    ...
                    if status == "answering" and not record.get("reminded") and now >= int(record.get("reminder_ts") or 0):
                        ...
                        try:
                            await application.bot.send_message(
                                chat_id=chat_id,
                                text=VALIDATION_REMINDER_MESSAGE.replace("{mention}", mention).replace("{name}", h(record.get("name", ""))),
                                parse_mode=ParseMode.HTML,
                            )
                        except Exception:
                    ...
                    if status == "answering" and now >= int(record.get("deadline_ts") or 0):
                        ...
                        try:
                            await application.bot.send_message(
                                chat_id=chat_id,
                                text=VALIDATION_TIMEOUT_MESSAGE.replace("{mention}", mention).replace("{name}", h(record.get("name", ""))),
                                parse_mode=ParseMode.HTML,
                            )
                        except Exception:
                            pass
                        if VALIDATION_KICK_IF_TIMEOUT:
                            try:
...
async def validation_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        f"Activo: {VALIDATION_ENABLED}\n"
        f"Preguntas: {len(VALIDATION_QUESTIONS)}\n"
        f"Tiempo límite: {VALIDATION_TIMEOUT_MINUTES} min\n"
        f"Recordatorio: {VALIDATION_REMINDER_MINUTES} min\n"
        f"Expulsar al agotar tiempo: {VALIDATION_KICK_IF_TIMEOUT}\n\n"
