...
async def daily_phrase_maybe_send(application: Application, chat_id: int, now_ts: int) -> None:
    ...
    if bool(cfg.get("daily_phrase_pin", False)):
        try:
            ...
            pass
    cfg["daily_phrase_last_key"] = _local_day_key_v15(now_ts)
    save_all_states()
...
async def resumen_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        pass
    wait = await update.message.reply_text(f"🧠 Preparando resumencito divertido de los últimos {limit} mensajes...")
    users: Dict[str, int] = {}; words: Dict[str, int] = {}; sample=[]; media=0; questions=0
    stop = set("que de la el en y a los las un una unos unas por para con del al se es no si lo me te tu yo mi su nos os como más mas pero porque ya hoy ayer mañana aqui aquí este esta esto eso esa ese hay muy todo todos todas cuando donde quien cuál cada algo nada".split())
    try:
        async for m in VOICE.client.iter_messages(chat_id, limit=limit):
            sender = await m.get_sender() if hasattr(m, "get_sender") else None
            name = (getattr(sender, "first_name", "") or getattr(sender, "username", "") or "Alguien") if sender else "Alguien"
            users[name] = users.get(name, 0) + 1
            if getattr(m, "media", None): media += 1
            txt = (getattr(m, "raw_text", "") or "").strip()
            if txt:
                questions += txt.count("?") + txt.count("¿")
                if len(txt) > 18 and len(sample) < 12:
                    sample.append((name, _re_v7.sub(r"\s+", " ", txt)[:140]))
                for w in _re_v7.findall(r"[A-Za-zÁÉÍÓÚáéíóúÑñ]{4,}", txt.lower()):
                    if w not in stop:
                        words[w] = words.get(w, 0) + 1
        top_users = sorted(users.items(), key=lambda kv: kv[1], reverse=True)[:5]
        top_words = [w for w,_ in sorted(words.items(), key=lambda kv: kv[1], reverse=True)[:8]]
        protagonist = top_users[0][0] if top_users else "el grupo"
        sidekick = top_users[1][0] if len(top_users) > 1 else "alguien misterioso"
        theme = ", ".join(h(w) for w in top_words[:5]) or "salseo, risas y mensajes random"
        lines = ["🎬 <b>RESUMENCITO DEL GRUPO</b>", ""]
        lines.append(f"En los últimos <b>{sum(users.values())}</b> mensajes, <b>{h(protagonist)}</b> entró fuerte en escena, mientras <b>{h(sidekick)}</b> hacía de coprotagonista oficial.")
        lines.append(f"La trama giró alrededor de: <b>{theme}</b>.")
        lines.append(f"Hubo <b>{questions}</b> preguntas, <b>{media}</b> momentos con multimedia y varias señales claras de que aquí hay vida.")
        if sample:
            lines.append("\n🍿 <b>Escenas destacadas:</b>")
            for name, txt in sample[:5]:
                lines.append(f"• <b>{h(name)}</b>: “{h(txt)}”")
        lines.append("\n😂 <b>Diagnóstico:</b> el grupo sigue con pulso. Falta que alguien tire una /preguntita, un /retito o que el DJ se venga arriba.")
        await wait.edit_text("\n".join(lines), parse_mode=ParseMode.HTML)
    except Exception:
