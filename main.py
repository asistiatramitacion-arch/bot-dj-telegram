...
def hot_activity_remember(chat_id: int, user) -> None:
    ...
    for uid in list(users.keys()):
        if int(users[uid].get("ts", 0)) < cutoff:
            users.pop(uid, None)


def hot_register_question(chat_id: int, message_id: int, target_user) -> None:
...
async def preguntita_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await register_bot_message(chat_id, msg.message_id)
    hot_register_question(chat_id, msg.message_id, target)
...
def hot_config_text(chat_id: int) -> str:
    ...
    return (
        ...
        f"Borrado módulo HOT: <b>{bool_label(cfg.get('hot_auto_delete_enabled', True))}</b>\n"
        f"Borrar tras: <b>{int(cfg.get('hot_auto_delete_seconds', 45) or 45)}s</b>\n"
...
def hot_config_markup(chat_id: int) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton("Auto 60s", callback_data="hot|interval|60"),
         InlineKeyboardButton("Auto 180s", callback_data="hot|interval|300"),
         InlineKeyboardButton("Auto 300s", callback_data="hot|interval|700")],
        [InlineKeyboardButton("Borrado HOT " + ("ON ✅" if autodel else "OFF ❌"), callback_data="hot|toggle|delete")],
        [InlineKeyboardButton("Borrar 30s", callback_data="hot|delete_after|30"),
         InlineKeyboardButton("45s", callback_data="hot|delete_after|45"),
...
async def addpregunta_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if parts and parts[0].isdigit() and len(parts) > 1:
        ...
        return
    HOT_PENDING_ADD[update.effective_user.id] = {"chat_id": chat_id, "mode": "single", "questions": [raw]}
...
async def addmasivo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await is_admin(context, chat_id, update.effective_user.id):
        await update.message.reply_text("Solo administradores.")
        return
    HOT_PENDING_ADD[update.effective_user.id] = {"chat_id": chat_id, "mode": "bulk_wait_text", "questions": []}
...
async def hot_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if pending and pending.get("mode") == "bulk_wait_text" and int(pending.get("chat_id")) == chat_id:
    ...
    if len(text) < 3:
        return
    elapsed = int(time.time()) - int(data.get("ts", time.time()))
    points = 2 if elapsed <= 10 else 1
    total = hot_add_points(chat_id, user, points)
    active.pop(int(reply.message_id), None)
    msg = await update.message.reply_html(f"🔥 <b>{h(display_name(user))}</b> +{points} pts\n🏆 Total: <b>{total}</b>")
    await register_bot_message(chat_id, msg.message_id)
        ...
        for chat_id in list(STATE_CACHE.keys()):
            try:
                ...
                if now - int(cfg.get("hot_last_auto_ts", 0) or 0) < interval:
                    continue
                users = [u for u in HOT_RECENT_ACTIVITY.get(chat_id, {}).values() if now - int(u.get("ts", 0)) <= 180]
                if not users:
                    continue
                ...
                fake.last_name = ""
                fake.username = None
                hot_register_question(chat_id, msg.message_id, fake)
