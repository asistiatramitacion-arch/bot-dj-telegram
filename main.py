...
def elapsed_seconds(state: ChatState) -> int:
    if state.play_started_at is None:
        return 0
    import time as _time
    return max(0, int(_time.time() - state.play_started_at))
...
async def _auto_next_monitor(chat_id: int, track_token: str) -> None:
    ...
    try:
        while True:
            ...
            trigger = FORCE_NEXT_SECONDS
            if state.auto_next_offset < 0:
                trigger = max(FORCE_NEXT_SECONDS, abs(int(state.auto_next_offset)))

            # Vía principal: cálculo por tiempo restante.
            if remaining is not None:
                unknown_seen_cycles = 0
                if remaining <= trigger:
                    await simulate_panel_next(chat_id, reason=f"remaining<={trigger}")
                    return
                if remaining <= 0:
                    await simulate_panel_next(chat_id, reason="remaining<=0")
                    return
                continue

            # Vía de respaldo: si la duración está en la pista pero el panel no la está mostrando bien,
            # usamos el tiempo transcurrido desde que se pulsó play.
            if int(track.duration or 0) > 0:
                unknown_seen_cycles = 0
                if elapsed >= max(0, int(track.duration) - trigger):
                    await simulate_panel_next(chat_id, reason=f"elapsed>={int(track.duration) - trigger}")
                    return
                continue

            # Último recurso: si el panel sigue en --:-- de forma sostenida, forzamos siguiente.
            unknown_seen_cycles += 1
            if unknown_seen_cycles >= 2:
                await simulate_panel_next(chat_id, reason="remaining=--:--")
                return
