...
# =========================
# V34 FIX: selector de grupos en privado
# =========================
# Algunos paneles privados muestran una lista de grupos y, si otro CallbackQueryHandler
# genérico intercepta antes o un callback lanza excepción, Telegram deja el botón
# "pensando". Este router de prioridad alta atiende solo selectores de grupo y
# evita duplicados parando la propagación.
from telegram.ext import ApplicationHandlerStop as _V34ApplicationHandlerStop

_old_admin_private_config_callback_v34 = admin_private_config_callback
_old_hotgroup_callback_router_v34 = hotgroup_callback_router
_old_djgroup_callback_router_v34 = djgroup_callback_router
_old_build_application_v34 = build_application

async def _v34_group_selector_guard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query:
        return
    data = query.data or ""
    if not (data.startswith("cfg|") or data.startswith("hotgroup|") or data.startswith("djgroup|")):
        return
    try:
        if data.startswith("cfg|"):
            await _old_admin_private_config_callback_v34(update, context)
        elif data.startswith("hotgroup|"):
            await _old_hotgroup_callback_router_v34(update, context)
        elif data.startswith("djgroup|"):
            await _old_djgroup_callback_router_v34(update, context)
    except Exception:
        logger.exception("V34: error en selector/configuración privada: %s", data)
        try:
            await query.answer("No pude abrir ese panel. Pulsa 🔄 Recargar o vuelve a /ajustes.", show_alert=True)
        except Exception:
            pass
    raise _V34ApplicationHandlerStop

# También hacemos el listado de grupos más robusto: si el título no está guardado,
# no falla; muestra el ID y entra igualmente.
def admin_private_chat_list_markup() -> InlineKeyboardMarkup:  # type: ignore[override]
    rows: List[List[InlineKeyboardButton]] = []
    try:
        chat_ids = known_admin_chats_for_private()
    except Exception:
        logger.exception("V34: no pude construir lista de grupos")
        chat_ids = []
    for chat_id in chat_ids:
        try:
            title = str(cfg_value(chat_id, "chat_title", "")) or str(chat_id)
            rows.append([InlineKeyboardButton(f"⚙️ {truncated_button_title(title, 40)}", callback_data=f"cfg|open|{int(chat_id)}")])
        except Exception:
            try:
                rows.append([InlineKeyboardButton(f"⚙️ {chat_id}", callback_data=f"cfg|open|{int(chat_id)}")])
            except Exception:
                pass
    rows.append([InlineKeyboardButton("🔄 Actualizar lista", callback_data="cfg|list")])
    rows.append([InlineKeyboardButton("❌ Cerrar", callback_data="cfg|close|0")])
    return InlineKeyboardMarkup(rows)

# Añadimos el handler de prioridad alta al build actual.
def build_application() -> Application:  # type: ignore[override]
    app = _old_build_application_v34()
    app.add_handler(CallbackQueryHandler(_v34_group_selector_guard, pattern="^(cfg|hotgroup|djgroup)\\|"), group=-100)
    return app

# Arranque real al final.
...
# =========================
# V35 FIX DEFINITIVO: selector de grupos /ajustes sin quedarse pensando
# =========================
# En algunos clientes el botón de selección de grupo quedaba en estado "cargando"
# porque el callback pasaba por routers genéricos o un edit fallaba sin pintar panel.
# Este router atiende directamente los botones críticos del selector y pinta el panel
# sin delegar en el router general. Solo si no es una acción de selector delega.
from telegram.ext import ApplicationHandlerStop as _V35ApplicationHandlerStop

_old_build_application_v35 = build_application
_old_admin_private_config_callback_v35 = admin_private_config_callback
_old_hotgroup_callback_router_v35 = hotgroup_callback_router
_old_djgroup_callback_router_v35 = djgroup_callback_router

async def _v35_safe_edit_or_reply(query, text: str, *, reply_markup=None, parse_mode=ParseMode.HTML) -> None:
    try:
        ...
        if "message is not modified" in low:
            try:
                await query.answer("Ya estás en ese panel.", show_alert=False)
            except Exception:
                pass
            return
        logger.exception("V35: no pude editar mensaje de selector: %s", e)
    except Exception:
        logger.exception("V35: error editando mensaje de selector")
    # Fallback: si no se puede editar, manda un mensaje nuevo en privado.
    try:
        await query.message.reply_html(text, reply_markup=reply_markup)
    except Exception:
        logger.exception("V35: tampoco pude responder con panel nuevo")

async def _v35_group_selector_direct_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query or not update.effective_user:
        return
    data = query.data or ""
    ...
    try:
        # Contestamos SIEMPRE al callback para que Telegram quite el spinner.
        try:
            await query.answer()
        except Exception:
            pass
        ...
        if not is_global_admin_user(update.effective_user.id):
            await query.answer("Solo administradores del bot.", show_alert=True)
            raise _V35ApplicationHandlerStop

        parts = data.split("|")

        # /ajustes: listado de grupos
        if data == "cfg|list" or (len(parts) >= 2 and parts[0] == "cfg" and parts[1] == "list"):
            await _v35_safe_edit_or_reply(
                query,
                "<b>Elige el grupo que quieres configurar:</b>",
        ...
        # /ajustes: cerrar selector
        if len(parts) >= 2 and parts[0] == "cfg" and parts[1] == "close":
            try:
                await query.message.delete()
            except Exception:
                await _v35_safe_edit_or_reply(query, "Panel cerrado.")
            raise _V35ApplicationHandlerStop

        # /ajustes: abrir grupo seleccionado
        if len(parts) >= 3 and parts[0] == "cfg" and parts[1] == "open":
            try:
                chat_id = int(parts[2])
            except Exception:
                await query.answer("Grupo inválido.", show_alert=True)
            ...
            await _v35_safe_edit_or_reply(
                query,
                admin_private_main_text(chat_id),
                reply_markup=admin_private_main_markup(chat_id),
            )
            raise _V35ApplicationHandlerStop

        # /ajustes: abrir bloque de un grupo
        if len(parts) >= 4 and parts[0] == "cfg" and parts[1] == "block":
            try:
                chat_id = int(parts[2])
                block = parts[3]
            except Exception:
                await query.answer("Grupo o bloque inválido.", show_alert=True)
                raise _V35ApplicationHandlerStop
            page = 0
            await _v35_safe_edit_or_reply(
                query,
                admin_private_block_text(chat_id, block, page),
                reply_markup=admin_private_block_markup(chat_id, block, page),
            )
            raise _V35ApplicationHandlerStop

        # Hot config: seleccionar grupo desde privado
        if len(parts) >= 3 and parts[0] == "hotgroup" and parts[1] == "select":
            try:
                chat_id = int(parts[2])
            except Exception:
                await query.answer("Grupo inválido.", show_alert=True)
        ...
        # DJ config: seleccionar grupo desde privado
        if len(parts) >= 3 and parts[0] == "djgroup" and parts[1] == "select":
            try:
                chat_id = int(parts[2])
            except Exception:
                await query.answer("Grupo inválido.", show_alert=True)
            ...
            await _v35_safe_edit_or_reply(
                query,
                dj_private_text(chat_id),
                reply_markup=dj_private_markup(chat_id),
            )
            raise _V35ApplicationHandlerStop

        # Para el resto de cfg/hotgroup/djgroup, delegamos pero paramos después.
        if data.startswith("cfg|"):
            await _old_admin_private_config_callback_v35(update, context)
        elif data.startswith("hotgroup|"):
            await _old_hotgroup_callback_router_v35(update, context)
        elif data.startswith("djgroup|"):
            await _old_djgroup_callback_router_v35(update, context)

    except _V35ApplicationHandlerStop:
        raise
    except Exception:
        logger.exception("V35: error en selector directo: %s", data)
        try:
            await query.answer("No pude abrir ese panel. Pulsa 🔄 Actualizar lista o vuelve a /ajustes.", show_alert=True)
        except Exception:
            pass
    raise _V35ApplicationHandlerStop

# Listado robusto: título guardado si existe, si no ID.
def admin_private_chat_list_markup() -> InlineKeyboardMarkup:  # type: ignore[override]
    rows: List[List[InlineKeyboardButton]] = []
    try:
        chat_ids = known_admin_chats_for_private()
    except Exception:
        logger.exception("V35: no pude construir lista de grupos")
        chat_ids = []
    for chat_id in chat_ids:
        try:
            title = str(cfg_value(int(chat_id), "chat_title", "")) or str(chat_id)
            rows.append([InlineKeyboardButton(f"⚙️ {truncated_button_title(title, 40)}", callback_data=f"cfg|open|{int(chat_id)}")])
        except Exception:
            continue
    rows.append([InlineKeyboardButton("🔄 Actualizar lista", callback_data="cfg|list")])
...
def build_application() -> Application:  # type: ignore[override]
    app = _old_build_application_v35()
    # Prioridad máxima. Así ningún router genérico se come el botón.
    app.add_handler(CallbackQueryHandler(_v35_group_selector_direct_router, pattern="^(cfg|hotgroup|djgroup)\\|"), group=-1000)
    return app
...
# =========================
# V36 FIX: reparación de configuración privada / grupos obsoletos
# =========================
# Este parche NO borra /data/state.json ni configuraciones de grupos reales.
# Solo evita que el selector privado muestre chats inválidos/privados/positivos
# que se hubieran guardado por error, y añade /fixprivado para limpiar vínculos
# privados de HOT/DJ sin tocar recurrentes, listas, validación ni DJ.

_old_build_application_v36 = build_application
_old_admin_private_chat_list_markup_v36 = admin_private_chat_list_markup
_old_admin_private_config_command_v36 = admin_private_config_command
_old_group_selector_direct_router_v36 = _v35_group_selector_direct_router


def _v36_is_probable_group_chat_id(chat_id: Any) -> bool:
    try:
...
def _v36_known_group_chat_ids() -> List[int]:
    """Devuelve solo IDs que parecen grupos/supergrupos. Evita privados positivos."""
    result: List[int] = []
    try:
        for cid in list(STATE_CACHE.keys()):
            if _v36_is_probable_group_chat_id(cid):
...
# Sobrescribimos el listado general de /ajustes: nunca mostrar IDs positivos.
def known_admin_chats_for_private() -> List[int]:  # type: ignore[override]
    return _v36_known_group_chat_ids()
...
def admin_private_chat_list_markup() -> InlineKeyboardMarkup:  # type: ignore[override]
    ...
    for chat_id in known_admin_chats_for_private():
        try:
            title = str(cfg_value(int(chat_id), "chat_title", "") or "").strip() or str(chat_id)
            rows.append([InlineKeyboardButton(f"⚙️ {truncated_button_title(title, 40)}", callback_data=f"cfg|open|{int(chat_id)}")])
        except Exception:
            rows.append([InlineKeyboardButton(f"⚙️ {chat_id}", callback_data=f"cfg|open|{int(chat_id)}")])
    rows.append([InlineKeyboardButton("🧹 Reparar lista privada", callback_data="cfg|repair|0")])
    rows.append([InlineKeyboardButton("🔄 Actualizar lista", callback_data="cfg|list")])
    rows.append([InlineKeyboardButton("❌ Cerrar", callback_data="cfg|close|0")])
    return InlineKeyboardMarkup(rows)


async def _v36_repair_private_links(context: ContextTypes.DEFAULT_TYPE, user_id: int) -> str:
    """Limpia vínculos privados rotos y refresca títulos. No elimina configuraciones de grupos."""
    removed_hot: List[str] = []
    removed_dj: List[str] = []
    refreshed: List[str] = []

    async def is_valid_group(cid: int) -> Tuple[bool, str]:
        if not _v36_is_probable_group_chat_id(cid):
            return False, ""
        try:
            chat = await context.bot.get_chat(int(cid))
            ctype = str(getattr(chat, "type", "") or "")
            if ctype not in ("group", "supergroup"):
                return False, getattr(chat, "title", "") or str(cid)
            title = getattr(chat, "title", "") or str(cid)
            try:
                cfg_set(int(cid), "chat_title", title)
            except Exception:
                pass
            refreshed.append(title)
            return True, title
        except Exception:
            # Si no puede consultar el chat pero es negativo y ya tenía título, lo dejamos.
            title = str(cfg_value(int(cid), "chat_title", "") or "").strip()
            if title:
                return True, title
            return False, ""

    # Limpieza HOT_PRIVATE_GROUPS
    try:
        rec = hot_user_group_record(user_id)
        groups = rec.get("groups", {}) if isinstance(rec, dict) else {}
        if isinstance(groups, dict):
            for key in list(groups.keys()):
                try:
                    ...
                    removed_hot.append(str(key)); groups.pop(key, None); continue
                ok, title = await is_valid_group(cid)
                if not ok:
                    removed_hot.append(str(key)); groups.pop(key, None)
                elif title:
                    groups[str(cid)] = title
            selected = rec.get("selected")
            if selected is not None:
                try:
                    if str(int(selected)) not in groups:
                        rec["selected"] = None
                except Exception:
                    rec["selected"] = None
            hot_save_private_groups()
    except Exception:
        logger.exception("V36: no pude reparar HOT_PRIVATE_GROUPS")

    # Limpieza DJ_PRIVATE_GROUPS
    try:
        key = str(int(user_id))
        rec = DJ_PRIVATE_GROUPS.setdefault(key, {"selected": None, "groups": {}})
        groups = rec.get("groups", {}) if isinstance(rec, dict) else {}
        if isinstance(groups, dict):
            for key2 in list(groups.keys()):
                try:
                    cid = int(key2)
                except Exception:
                    removed_dj.append(str(key2)); groups.pop(key2, None); continue
                ok, title = await is_valid_group(cid)
                if not ok:
                    removed_dj.append(str(key2)); groups.pop(key2, None)
                elif title:
                    groups[str(cid)] = title
            selected = rec.get("selected")
            if selected is not None:
        ...
        logger.exception("V36: no pude reparar DJ_PRIVATE_GROUPS")

    save_all_states()
    lines = ["🧹 <b>Reparación privada completada</b>", ""]
    lines.append(f"Grupos válidos en /ajustes: <b>{len(known_admin_chats_for_private())}</b>")
    lines.append(f"HOT limpiados: <b>{len(removed_hot)}</b>")
    lines.append(f"DJ limpiados: <b>{len(removed_dj)}</b>")
    if removed_hot or removed_dj:
        lines.append("")
        lines.append("He quitado vínculos privados inválidos/antiguos. No he borrado configuraciones reales de grupos.")
    lines.append("")
    lines.append("Ahora prueba <code>/ajustes</code>. Si faltara algún grupo, entra en ese grupo y ejecuta <code>/plan</code>, <code>/hotgrupo</code> o <code>/djgrupo</code> según toque.")
    return "\n".join(lines)


async def fixprivado_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo ADMIN_IDS puede reparar la configuración privada.")
        return
    text = await _v36_repair_private_links(context, update.effective_user.id)
    await update.message.reply_html(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚙️ Abrir ajustes", callback_data="cfg|list")]]))


async def admin_private_config_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    if not update.effective_user or not update.message:
        return
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo ADMIN_IDS puede abrir la configuración privada.")
    ...
    if not chats:
        await update.message.reply_html(
            "Todavía no tengo grupos válidos registrados para configurar.\n\n"
            "Entra en el grupo y ejecuta <code>/plan</code>. Para HOT usa <code>/hotgrupo</code> y para DJ <code>/djgrupo</code>.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🧹 Reparar lista privada", callback_data="cfg|repair|0")]])
        )
        return
    await update.message.reply_html("<b>Elige el grupo que quieres configurar:</b>", reply_markup=admin_private_chat_list_markup())


async def _v36_group_selector_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query or not update.effective_user:
        return
    data = query.data or ""
    ...
    try:
        ...
        if data == "cfg|repair|0":
            text = await _v36_repair_private_links(context, update.effective_user.id)
            await _v35_safe_edit_or_reply(query, text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚙️ Volver a elegir grupo", callback_data="cfg|list")]]))
            raise _V35ApplicationHandlerStop

        if data == "cfg|list":
            await _v35_safe_edit_or_reply(query, "<b>Elige el grupo que quieres configurar:</b>", reply_markup=admin_private_chat_list_markup())
            raise _V35ApplicationHandlerStop
        ...
        if data.startswith("cfg|open|"):
            try:
                ...
                await query.answer("Grupo inválido.", show_alert=True)
                raise _V35ApplicationHandlerStop
            if not _v36_is_probable_group_chat_id(chat_id):
                await query.answer("Ese ID no parece un grupo. Pulsa 🧹 Reparar lista privada.", show_alert=True)
                raise _V35ApplicationHandlerStop
            # Refrescamos título si se puede, sin impedir abrir si falla.
            try:
                chat = await context.bot.get_chat(chat_id)
            ...
            try:
                ...
                logger.exception("V36: error construyendo panel principal para %s", chat_id)
                text = (
                    f"⚙️ <b>Configuración del grupo</b>\n\n"
                    f"Grupo: <b>{h(cfg_value(chat_id, 'chat_title', '') or chat_id)}</b>\n\n"
                    "No pude construir el panel completo por una configuración antigua. Pulsa reparar o vuelve a vincular el grupo."
                )
                markup = InlineKeyboardMarkup([
                    [InlineKeyboardButton("🧹 Reparar lista privada", callback_data="cfg|repair|0")],
                    [InlineKeyboardButton("🔙 Elegir otro grupo", callback_data="cfg|list")],
                ])
            await _v35_safe_edit_or_reply(query, text, reply_markup=markup)
            raise _V35ApplicationHandlerStop

        if data.startswith("cfg|block|"):
            try:
                chat_id = int(parts[2]); block = parts[3]
            except Exception:
                await query.answer("Grupo o bloque inválido.", show_alert=True)
                raise _V35ApplicationHandlerStop
            try:
                await _v35_safe_edit_or_reply(query, admin_private_block_text(chat_id, block, 0), reply_markup=admin_private_block_markup(chat_id, block, 0))
            except Exception:
                logger.exception("V36: error abriendo bloque %s en %s", block, chat_id)
                await query.message.reply_html("❌ No pude abrir ese bloque. Prueba 🧹 Reparar lista privada desde /ajustes.")
            raise _V35ApplicationHandlerStop

        # Para HOT y DJ delegamos al router directo V35, que ya está reforzado.
        await _old_group_selector_direct_router_v36(update, context)
    except _V35ApplicationHandlerStop:
        raise
    except Exception:
        logger.exception("V36: error en selector privado: %s", data)
        try:
            await query.answer("No pude abrir el panel. Usa /fixprivado y vuelve a probar.", show_alert=True)
        except Exception:
            pass
    raise _V35ApplicationHandlerStop
...
def build_application() -> Application:  # type: ignore[override]
    app = _old_build_application_v36()
    # Más prioritario que todos los routers previos. Este es el que manda.
    app.add_handler(CallbackQueryHandler(_v36_group_selector_router, pattern="^(cfg|hotgroup|djgroup)\\|"), group=-2000)
    app.add_handler(CommandHandler("fixprivado", fixprivado_command), group=-2000)
    return app
...
# =========================
# V37 FIX TOTAL: selector privado con callback propio (cfgsel)
# =========================
from telegram.ext import ApplicationHandlerStop as _V37ApplicationHandlerStop

_old_build_application_v37 = build_application
...
def admin_private_chat_list_markup() -> InlineKeyboardMarkup:  # type: ignore[override]
    ...
    try:
        ...
        logger.exception("V37: no pude construir lista de grupos")
        chat_ids = []
    for chat_id in chat_ids:
        try:
            cid = int(chat_id)
            if not _v36_is_probable_group_chat_id(cid):
                continue
            title = str(cfg_value(cid, "chat_title", "")) or str(cid)
            rows.append([InlineKeyboardButton(f"⚙️ {truncated_button_title(title, 40)}", callback_data=f"cfgsel|{cid}")])
        except Exception:
            continue
    rows.append([InlineKeyboardButton("🔄 Actualizar lista", callback_data="cfgsel|list")])
    rows.append([InlineKeyboardButton("🧹 Reparar lista privada", callback_data="cfgsel|repair")])
    rows.append([InlineKeyboardButton("❌ Cerrar", callback_data="cfgsel|close")])
    return InlineKeyboardMarkup(rows)


async def _v37_edit_or_new(query, text: str, *, reply_markup=None, parse_mode=ParseMode.HTML) -> None:
    try:
        await query.edit_message_text(text, parse_mode=parse_mode, reply_markup=reply_markup)
        return
    except BadRequest as e:
        ...
        if "message is not modified" in low:
            try:
                await query.answer("Panel actualizado.", show_alert=False)
            except Exception:
                pass
            return
        logger.warning("V37: no pude editar selector (%s). Enviaré panel nuevo.", e)
    except Exception:
        logger.exception("V37: error editando selector. Enviaré panel nuevo.")
    try:
        if query.message:
            await query.message.reply_html(text, reply_markup=reply_markup)
    except Exception:
        logger.exception("V37: tampoco pude mandar panel nuevo")


async def _v37_open_admin_main(query, context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> None:
    if not _v36_is_probable_group_chat_id(chat_id):
        await query.answer("Ese ID no parece un grupo. Pulsa reparar lista privada.", show_alert=True)
        return
    try:
    ...
    try:
        ...
        logger.exception("V37: no pude construir el panel principal de %s", chat_id)
        text = (
            f"⚙️ <b>Configuración del grupo</b>\n\n"
            f"Grupo: <b>{h(cfg_value(chat_id, 'chat_title', '') or chat_id)}</b>\n\n"
            "No pude construir el panel completo. Prueba a reparar la lista privada."
        )
        markup = InlineKeyboardMarkup([
            [InlineKeyboardButton("🧹 Reparar lista privada", callback_data="cfgsel|repair")],
            [InlineKeyboardButton("🔙 Elegir otro grupo", callback_data="cfgsel|list")],
        ])
    await _v37_edit_or_new(query, text, reply_markup=markup)


async def _v37_private_selector_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query or not update.effective_user:
        return
    data = query.data or ""
    if not (
        data.startswith("cfgsel|")
        or data.startswith("cfg|open|")
        or data == "cfg|list"
        or data.startswith("cfg|close")
        or data.startswith("cfg|block|")
    ):
        return
    try:
        try:
            await query.answer("Abriendo…", show_alert=False)
        except Exception:
            pass
        if not is_global_admin_user(update.effective_user.id):
            await query.answer("Solo ADMIN_IDS puede configurar.", show_alert=True)
            raise _V37ApplicationHandlerStop
        parts = data.split("|")
        if data in ("cfgsel|list", "cfg|list"):
            await _v37_edit_or_new(query, "<b>Elige el grupo que quieres configurar:</b>", reply_markup=admin_private_chat_list_markup())
            raise _V37ApplicationHandlerStop
        if data == "cfgsel|repair":
            text2 = await _v36_repair_private_links(context, update.effective_user.id)
            await _v37_edit_or_new(query, text2, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚙️ Volver a elegir grupo", callback_data="cfgsel|list")]]))
            raise _V37ApplicationHandlerStop
        if data in ("cfgsel|close", "cfg|close|0") or data.startswith("cfg|close"):
            try:
                await query.message.delete()
            except Exception:
                await _v37_edit_or_new(query, "Panel cerrado.")
            raise _V37ApplicationHandlerStop
        if data.startswith("cfgsel|") and len(parts) >= 2:
            try:
                chat_id = int(parts[1])
            except Exception:
                await query.answer("Grupo inválido.", show_alert=True)
                raise _V37ApplicationHandlerStop
            await _v37_open_admin_main(query, context, chat_id)
            raise _V37ApplicationHandlerStop
        if data.startswith("cfg|open|") and len(parts) >= 3:
            ...
            await _v37_open_admin_main(query, context, chat_id)
            raise _V37ApplicationHandlerStop
        if data.startswith("cfg|block|") and len(parts) >= 4:
            try:
                chat_id = int(parts[2])
                block = parts[3]
            except Exception:
                await query.answer("Grupo o bloque inválido.", show_alert=True)
                raise _V37ApplicationHandlerStop
            await _v37_edit_or_new(query, admin_private_block_text(chat_id, block, 0), reply_markup=admin_private_block_markup(chat_id, block, 0))
            raise _V37ApplicationHandlerStop
    except _V37ApplicationHandlerStop:
        raise
    except Exception:
        logger.exception("V37: error total en selector privado: %s", data)
        try:
            await query.answer("No pude abrir el panel. Ejecuta /fixprivado y vuelve a probar.", show_alert=True)
        except Exception:
            pass
        raise _V37ApplicationHandlerStop
...
async def admin_private_config_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:  # type: ignore[override]
    ...
    if not chats:
        await update.message.reply_html(
            "Todavía no tengo grupos válidos registrados para configurar.\n\n"
            "Entra en el grupo y ejecuta <code>/plan</code>. Para HOT usa <code>/hotgrupo</code> y para DJ <code>/djgrupo</code>.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🧹 Reparar lista privada", callback_data="cfgsel|repair")]])
        )
        return
    await update.message.reply_html("<b>Elige el grupo que quieres configurar:</b>", reply_markup=admin_private_chat_list_markup())
...
def build_application() -> Application:  # type: ignore[override]
    app = _old_build_application_v37()
    app.add_handler(CallbackQueryHandler(_v37_private_selector_router, pattern="^(cfgsel|cfg\\|open|cfg\\|list|cfg\\|close|cfg\\|block)"), group=-3000)
    return app
...
# =========================
# V38 FIX DEFINITIVO: /ajustes privado directo y selector independiente (cfgx)
# Motivo: evitar que callbacks antiguos cfg| / cfgsel| se queden interceptados.
# =========================
from telegram.ext import ApplicationHandlerStop as _V38ApplicationHandlerStop

_old_build_application_v38 = build_application


def _v38_valid_private_chats() -> List[int]:
    """Devuelve solo IDs de grupos/supergrupos registrados, sin privados ni basura."""
    try:
        load_all_states()
    except Exception:
        pass
    ...
    try:
        for cid in known_admin_chats_for_private():
            try:
                ...
                continue
            # En Telegram, grupos/supergrupos suelen ser negativos. Privados son positivos.
            if icid >= 0:
                continue
            if icid not in out:
                out.append(icid)
    except Exception:
        logger.exception("V38: no pude leer grupos privados")
    return out


def _v38_group_title(chat_id: int) -> str:
    try:
        title = str(cfg_value(chat_id, "chat_title", "") or "").strip()
        return title or str(chat_id)
    except Exception:
        return str(chat_id)


def _v38_selector_markup() -> InlineKeyboardMarkup:
    rows: List[List[InlineKeyboardButton]] = []
    for cid in _v38_valid_private_chats():
        title = _v38_group_title(cid)
        rows.append([InlineKeyboardButton(f"⚙️ {truncated_button_title(title, 40)}", callback_data=f"cfgx|open|{cid}")])
    rows.append([InlineKeyboardButton("🔄 Actualizar", callback_data="cfgx|list|0")])
    rows.append([InlineKeyboardButton("🧹 Reparar", callback_data="cfgx|repair|0")])
    rows.append([InlineKeyboardButton("❌ Cerrar", callback_data="cfgx|close|0")])
    return InlineKeyboardMarkup(rows)


async def _v38_send_group_selector(message) -> None:
    chats = _v38_valid_private_chats()
    if not chats:
        await message.reply_html(
            "⚠️ <b>No encuentro grupos configurables válidos.</b>\n\n"
            "Haz esto:\n"
            "1) Entra en el grupo.\n"
            "2) Ejecuta <code>/plan</code>.\n"
            "3) Si quieres HOT: <code>/hotgrupo</code>.\n"
            "4) Si quieres DJ: <code>/djgrupo</code>.\n"
            "5) Vuelve aquí y usa <code>/ajustes</code>."
        )
        return
    await message.reply_html(
        "⚙️ <b>Elige el grupo que quieres configurar</b>\n\n"
        "Este selector es directo. Si uno falla, pulsa 🧹 Reparar y vuelve a probar.",
        reply_markup=_v38_selector_markup(),
    )


async def _v38_open_panel_by_chat_id(bot, private_chat_id: int, chat_id: int, *, reply_to_query=None) -> None:
    # Actualiza título si Telegram deja leerlo, pero no bloquea si falla.
    try:
        chat = await bot.get_chat(chat_id)
        title = getattr(chat, "title", "") or str(chat_id)
        cfg_set(chat_id, "chat_title", title)
    ...
    try:
        ...
        logger.exception("V38: no pude construir admin_private_main para %s", chat_id)
        text = (
            "⚙️ <b>Configuración del grupo</b>\n\n"
            f"Grupo: <b>{h(_v38_group_title(chat_id))}</b>\n"
            f"ID: <code>{chat_id}</code>\n\n"
            "No pude construir el panel completo. Pulsa reparar y prueba otra vez."
        )
        markup = InlineKeyboardMarkup([
            [InlineKeyboardButton("🧹 Reparar", callback_data="cfgx|repair|0")],
            [InlineKeyboardButton("🔙 Elegir grupo", callback_data="cfgx|list|0")],
        ])

    # Para evitar el bug de spinner/edición, siempre mandamos un panel nuevo.
    try:
        await bot.send_message(
        ...
        logger.exception("V38: no pude enviar panel privado para %s", chat_id)
        if reply_to_query:
            try:
                await reply_to_query.answer("No pude enviar el panel. Mira logs de Railway.", show_alert=True)
            except Exception:
                pass


async def ajustes_directo_v38(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo ADMIN_IDS puede abrir la configuración privada.")
        raise _V38ApplicationHandlerStop
    if update.effective_chat and update.effective_chat.type != "private":
        await update.message.reply_text("Escríbeme /ajustes por privado para configurar el grupo sin ensuciar el chat.")
        raise _V38ApplicationHandlerStop

    await _v38_send_group_selector(update.message)
    raise _V38ApplicationHandlerStop


async def fixprivado_directo_v38(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    if not is_global_admin_user(update.effective_user.id):
        await update.message.reply_text("Solo ADMIN_IDS puede reparar el privado.")
        raise _V38ApplicationHandlerStop
    try:
        text = await _v36_repair_private_links(context, update.effective_user.id)
    except Exception:
        logger.exception("V38: error en reparación privada")
        text = "⚠️ No pude reparar automáticamente. Revisa logs de Railway."
    await update.message.reply_html(text, reply_markup=_v38_selector_markup())
    raise _V38ApplicationHandlerStop


async def cfgx_router_v38(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query or not update.effective_user:
        return
    data = query.data or ""
    if not data.startswith("cfgx|"):
        return
    try:
        await query.answer("Abriendo…", show_alert=False)
    except Exception:
    ...
    try:
        ...
        if action == "repair":
            try:
                ...
                logger.exception("V38: error reparando desde botón")
                text = "⚠️ No pude reparar automáticamente. Mira logs de Railway."
            try:
                await query.message.reply_html(text, reply_markup=_v38_selector_markup())
            except Exception:
                pass
            raise _V38ApplicationHandlerStop
        ...
        if action == "open":
            try:
                ...
                raise _V38ApplicationHandlerStop
            if chat_id >= 0:
                await query.answer("Ese ID no parece un grupo. Pulsa reparar.", show_alert=True)
                raise _V38ApplicationHandlerStop
            await _v38_open_panel_by_chat_id(context.bot, update.effective_user.id, chat_id, reply_to_query=query)
            raise _V38ApplicationHandlerStop

    except _V38ApplicationHandlerStop:
        raise
    except Exception:
        logger.exception("V38: error en cfgx router data=%s", data)
        try:
            await query.answer("Error abriendo el panel. Revisa logs de Railway.", show_alert=True)
        except Exception:
            pass
        raise _V38ApplicationHandlerStop
...
def build_application() -> Application:  # type: ignore[override]
    app = _old_build_application_v38()
    # Prioridad máxima: estos handlers se ejecutan antes que los antiguos.
    app.add_handler(CommandHandler(["ajustes", "config", "configuracion", "admin"], ajustes_directo_v38), group=-5000)
    app.add_handler(CommandHandler("fixprivado", fixprivado_directo_v38), group=-5000)
    app.add_handler(CallbackQueryHandler(cfgx_router_v38, pattern="^cfgx\\|"), group=-5000)
    return app
